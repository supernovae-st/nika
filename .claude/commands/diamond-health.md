...
grep -c '"tools/' Cargo.toml 2>/dev/null || echo 0
...
# 3. Files >1500 LOC
find tools -name '*.rs' -not -path '*/target/*' | xargs wc -l 2>/dev/null | awk '$1 > 1500 && $2 !~ /total/ {print "⚠️", $1, $2}'

# 4. Unwrap count in src/ (prod only, skip tests)
rg '\.unwrap\(\)' tools/*/src --type rust -g '!*test*' 2>/dev/null | wc -l
...
# 7. Largest file
find tools -name '*.rs' -not -path '*/target/*' | xargs wc -l 2>/dev/null | sort -rn | head -3
```
