...
| S18 | 2026-04-12 | 33 (+1 nika-security) | 36 | **BIGGEST SINGLE-SESSION ENGINE REDUCTION** (−3,304 LOC, surpasses DX-2's −990). Headline A1 + bundled C6 + C7 + Z18-stale closure + DX-1 mop-up. **4-agent Phase 1 review in parallel** found the code-explorer's scope was 10× the handoff estimate (2,463 + 549 LOC of pure code, not 200) and overrode the architect's "merge into nika-policy" recommendation after verifying nika-policy's dep profile pulls tokio+reqwest+parking_lot+url. **11 commits:** (1) `07f63fcd3` move `redact_secrets` engine util → `nika-core::util::redact` (L0) + 6 new provider patterns (Replicate/Mistral/fal.ai prefix + Cohere/ElevenLabs/Together context-anchored with no-false-positive guards) — closes Z13. (2) `b4e471f84` `nika-security` L1 crate skeleton. (3) `338570ea2` **headline** move `runtime/security.rs` (2,463 LOC) into `nika-security` with new `SecurityError` enum (matches `NikaError::BlockedCommand` Display byte-for-byte, `#[non_exhaustive]` per invariant #25) + `From<SecurityError> for NikaError` engine impl with wildcard arm. (4) `b068fc4fe` move `io/security.rs` (549 LOC) → `nika-security::path` module; `SecurityError::ArtifactPath` variant → `NikaError::ArtifactPathError` (NIKA-280). (5) `33741ff11` add `nika_core::util::redact_value(Value) -> Value` helper, wire into `nika-verb-invoke::run` for `EventKind::McpInvoke` params redaction + golden test — closes Z12. (6) `0cf8cd6c8` delete 2 duplicate infer mock tests (covered by W16-B1 golden + verb-infer golden). (7) `d78c71271` move `shlex`+`flate2` to `[dev-dependencies]`. (8) `7073bf81f` MockBuiltinRouter: `BuiltinErrorSpec` private enum + `err_denied`/`err_invalid_args`/`err_assertion_failed` constructors + `impl Default` + `with_known(impl IntoIterator<Item = impl Into<String>>)`. (9) `ddf4ef9aa` `rig_agent_loop` flatten via `Option::filter` (−8 LOC, removes dead else). (10) `c8d8a51ff` remove redundant `#[allow(clippy::too_many_arguments)]` + fix a `wildcard_enum_match_arm` in test code. (11) `3532f3c1b` `nika-cli` dedup: `collect_shell_escape_warnings` helper for 2 sites in `check.rs` + `new_spinner` helper for 2 sites in `verbs.rs`. **Engine 146,656 → 143,352 LOC (−3,304 net).** Tests 10,913 → 10,922 (+9 including 25 new redact tests + 22 new path tests + 5 new mock error ctor tests + 1 Z12 golden, minus 2 duplicate infer deletions). Clippy 0 across `--all-features`. **D/A ratio ≈ 23×.** Zombies closed: Z12 (McpInvoke redaction), Z13 (6 provider patterns), Z18 (stale — was already AGPL via workspace inheritance). Zombie Z17 partial advance (one of 9 files >1500 LOC resolved). **Deferred to S19:** 5 rust-pro quality wins (let-else, inline fmt args, byte-slice contains_unquoted, Cow<str> basename, ASCII fast-path NFKC) held back from the move commit for bisect-cleanliness. **Not a pure duplicate:** nika-mcp's `validate_env_vars` has different `BLOCKED_ENV_VARS` scope (session hygiene HOME/USER/SHELL/LANG) than nika-security's (library injection LD_PRELOAD/BASH_ENV/PATH); closed without change. |

**Note on crate counting**: 33 = diamond-participating crates (L0–L5) post-S18 (+1 nika-security). The total of 36 includes 3 outside-the-diamond workspace members: `nika-napi`, `nika-py` (FFI shims), `nika-macros` (proc-macro support). Both numbers are correct depending on whether you're measuring architectural surface (33) or workspace size (36). Authoritative post-S18: **33 diamond, 36 total**.
