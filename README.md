...
is kept as a private reference on `brouillon`; `main` carries the clean-slate
Diamond rebuild from **v0.80.0-alpha** onward.

```mermaid
timeline
    title Nika version lineage
    Early engine (public · 96 tags) : v0.1.0 first prototype : v0.28.0 pre-launch
    Legacy monolith (private reference) : v0.29 → v0.79.3 : 138K-LOC nika-engine on brouillon
    Diamond rebuild (public · main · orphan) : v0.80.0-alpha : clean-slate 42-crate craft, 12-gate admission
```
