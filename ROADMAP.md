...
- **nika-catalog** — static catalogs with phf+unicase lookup (226 tests after 4B)
  ...
  - 105 MCP servers, **32 LLM providers**, 13 embeddings, 63 builtins, 65 transforms
    *(this is the `nika-catalog` **inventory** — what the engine has metadata for. Distinct from the `nika-spec` **v0.1 stdlib contract** of 13 providers · 22 builtins · 9 extract modes that a conformant engine must support · the extra builtins are media/opt-in, deferred to stdlib v0.x. The 63→22 catalog reconciliation was D-2026-05-26-N5/N6 + ADR-086/087/088 Rams sweep 2026-05-27.)*
