    ...
    W: WorkflowIntrospect,
{
    ...
    async fn route(
        ...
        jq_clock: JqClock,
    ) -> Result<(BuiltinOutcome, Option<String>), ToolExecError> {
        let args = args
            .as_object()
        ...
        // A builtin's non-fatal diagnostic (today: the directories a
        // `nika:glob` left out) rides beside the outcome — the value a
        // consumer receives never changes.
        let mut warning = None;
        let routed = match short {
            // core 6
            ...
            "glob" => Ok(self.guarded_glob(args).await.map(|globbed| {
                warning = globbed.warning;
                globbed.paths
            })),
            // data 9
        ...
        routed.map(|outcome| (outcome, warning))
    }
    ...
    /// The value is the file list; the report of the directories the walk
    /// left out rides beside it (the OBS-E `warning` · V9 wave 3 p10).
    async fn guarded_glob(&self, args: &Args) -> Result<file::Globbed, BuiltinFailure> {
        let root = args
            .get("pattern")
            .and_then(serde_json::Value::as_str)
        ...
        self.fs_boundary
            ...
            .await?;
        file::glob_reported(&self.judged(), args).await
    }
}
    ...
    W: WorkflowIntrospect,
{
    async fn execute(&self, call: ToolCall) -> Result<ToolResult, ToolExecError> {
        ...
        let (outcome, warning) = self.route(&call.name, &call.input, jq_clock).await?;
        let result = render(call.id.as_str(), outcome);
        // A builtin's non-fatal diagnostic rides the seam's own lane —
        // beside the value, never inside it (`ToolResult::warning`).
        Ok(match warning {
