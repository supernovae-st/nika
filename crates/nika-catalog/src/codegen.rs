...
//! JSON-Schema codegen primitives derived from the catalog.
//!
...
use crate::all_builtins;

/// Build the closed-enum JSON-Schema fragment for the `nika:<name>` branch
/// of the workflow's `invoke.tool` field.
///
/// Reads [`all_builtins`] at call time · the schema is automatically
/// up-to-date with the engine catalog (no-drift property · **structural**
/// · not discipline). The entries are sorted alphabetically (matches
/// `ALL_BUILTINS` binary-search invariant).
///
/// # Returned shape
///
/// ```jsonc
/// {
///   "type": "string",
///   "description": "`nika:*` builtin · 28 canonical per stdlib v0.1",
///   "enum": [ "nika:assert", "nika:compose", … ]
/// }
/// ```
///
/// # Cross-reference
...
/// - `nika/spec/stdlib/builtins-v0.1.md` (the canonical 28 source-of-truth)
#[must_use]
