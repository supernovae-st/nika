...
pub fn validate_catalog_integrity() -> Vec<CatalogError> {
    use crate::data::{ALL_MCP_SERVERS, ALL_PROVIDERS, builtins, transforms};

    let mut errors = Vec::new();
    validate_sorted(
        "builtins",
        builtins::ALL_BUILTINS.iter().map(|b| b.name),
        &mut errors,
    );
    validate_sorted(
        "transforms",
        transforms::ALL_TRANSFORMS.iter().map(|t| t.name),
        &mut errors,
    );
    ...
    validate_no_duplicates(
        "mcp_servers",
        ALL_MCP_SERVERS.iter().map(|s| s.id),
        &mut errors,
    );
