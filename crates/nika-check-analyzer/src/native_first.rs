...
/// The hint kind (agents route on it · `nika_check::hints` kind registry).
#[cfg(test)]
const KIND: &str = "native-first";

/// Local site shape used by this pass and its tests. The judgment crate
/// wraps [`classify_all`] into `nika_check::Hint` at the historical path.
#[cfg(test)]
...
#[cfg(test)]
fn scan(wf: &RawWorkflow) -> Vec<Hint> {
...
/// (`lints::native_first`) both classify here (re-exported at
/// `nika_check::native_first`).
#[must_use]
