...
//!
//! Usage: `cargo run -p nika-check --example check -- [--json] [--infer-permits] [--color=auto|always|never] workflow.nika.yaml`
//!
...
fn render_parse_error(
    ...
    let span = e
        .span()
        .map(|sp| nika_check::ByteSpan::new(sp.start.0, sp.end.0));
    if json_mode {
        let payload = serde_json::json!({
