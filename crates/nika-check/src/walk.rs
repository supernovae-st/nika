...
//! `for_each` island · what an action's text fields are). They emit no
//! hints and hold no policy, which is exactly why they belong beside the
//! hint lane rather than inside it: a traversal is reusable, a hint is a
//! judgement. The static read-path lint descended to the analysis
//! substrate (`nika-check-analyzer::read_paths` · 2026-09-13 · the 15k
//! wall) beside the resolver it reads through.
