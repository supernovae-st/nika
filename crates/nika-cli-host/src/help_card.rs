...
/// `nika compile --help` footer: the authoring door's exit ladder and its answer loop.
pub const COMPILE_EXITS: &str = "exit codes · 0 READY: the candidate is complete (written when a destination is named) · 2 the REQUEST: authoring is incomplete or refused (answer each `? key` with `--answer key=JSON_LITERAL` and compile again), an `--answer` without `=`, or a destination that is not a `*.nika` path · 3 the ENVIRONMENT: the destination exists (`--force` replaces it), an unreadable `--base`, an unwritable destination, or an authoring provider/limit failure · an --answer round replays the plan the previous round recorded under .nika/compile/ (zero provider calls); --fresh reads or samples the intent again";
