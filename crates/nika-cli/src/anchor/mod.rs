...
pub(crate) struct RunKeyMaterial {
...
impl RunKeyMaterial {
    ...
    /// the entry digest.
    pub(crate) fn sign(&self, artifact: &[u8; 32]) -> Result<[u8; 64], String> {
...
pub(crate) fn run_key_material(
    sk: &minisign::SecretKey,
    pk_box: &str,
) -> Result<RunKeyMaterial, String> {
...
mod tests {
    ...
    fn the_frozen_live_anchor_verifies_offline() {
        let super::super::verbs::trace_verify::Verdict::Intact { head: walked, .. } =
            super::super::verbs::trace_verify::walk(fixtures::JOURNAL)
        else {
