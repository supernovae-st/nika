...
use nika_core::ast::{TaskAction, Workflow};
use nika_core::dag::{FlowGraph, validate_use_wiring};
...
use nika_runtime::tools::PermissionMode;
...
async fn main() {
    ...
    let result = match cli.command {
        ...
        None => {
            #[cfg(feature = "tui")]
            {
                nika_tui::run_tui_standalone().await
            }
            #[cfg(not(feature = "tui"))]
            {
        ...
        // Chat mode
        #[cfg(feature = "tui")]
        Some(Commands::Chat { provider, model }) => nika_tui::run_tui_chat(provider, model).await,

        // Studio mode
        #[cfg(feature = "tui")]
        Some(Commands::Studio { workflow }) => nika_tui::run_tui_studio(workflow).await,
        ...
        Some(Commands::Tui { workflow }) => {
            ...
            match workflow {
                Some(path) => nika_tui::run_tui(&path).await,
                None => nika_tui::run_tui_standalone().await,
            }
        }
    };
...
fn handle_trace_command(action: TraceAction) -> Result<(), NikaError> {
    match action {
        TraceAction::List { limit } => {
            let traces = nika_core::list_traces()?;
