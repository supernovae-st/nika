...
/// The anatomy render over a pair the caller ALREADY holds.
///
/// Split from [`run`] for #1051: `--dry-run` used to render its human
/// card by calling `run(file, …)`, which re-read and re-checked the file
/// — so a `--model` swap reached the `plan_version: 1` object and never
/// the card a person reads. One projection, one pair, both surfaces.
