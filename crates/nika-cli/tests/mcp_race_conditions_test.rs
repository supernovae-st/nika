...
async fn test_arc_oncecell_required_for_await() {
    ...
    for i in 0..5 {
        let cache: Arc<DashMap<String, Arc<OnceCell<String>>>> = Arc::clone(&cache);
        ...
        handles.spawn(async move {
            ...
            let cell: Arc<OnceCell<String>> = cache
                .entry("key".to_string())
...
async fn test_concurrent_client_init() {
    ...
    for i in 0..10 {
        let cache: Arc<DashMap<String, Arc<OnceCell<Arc<McpClient>>>>> = Arc::clone(&client_cache);
        let counter = Arc::clone(&init_count);
        let barrier = Arc::clone(&barrier);
