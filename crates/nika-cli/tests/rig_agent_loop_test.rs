...
use nika_core::ast::AgentParams;
use nika_core::event::EventLog;
use nika_mcp::McpClient;
use nika_runtime::{RigAgentLoop, RigAgentLoopResult, RigAgentStatus};
...
async fn test_rig_agent_loop_emits_events() {
    ...
    let agent_events: Vec<_> = events
        .iter()
        .filter(|e| matches!(&e.kind, nika_core::event::EventKind::AgentTurn { .. }))
        .collect();
    assert!(
...
async fn test_workflow_event_log_captures_agent_lifecycle() {
    ...
    let kinds: Vec<String> = events
        .iter()
        .filter_map(|e| {
            if let nika_core::event::EventKind::AgentTurn { kind, .. } = &e.kind {
                Some(kind.clone())
            } else {
                None
            }
