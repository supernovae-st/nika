...
"#;
        let workflow = parse_workflow(yaml);
        let task = &workflow.tasks[0];
        assert!(matches!(task.action, nika_core::ast::TaskAction::Infer { .. }));
    }
...
"#;
        let workflow = parse_workflow(yaml);
        let task = &workflow.tasks[0];
        assert!(matches!(task.action, nika_core::ast::TaskAction::Exec { .. }));
    }
...
"#;
        let workflow = parse_workflow(yaml);
        let task = &workflow.tasks[0];
        assert!(matches!(task.action, nika_core::ast::TaskAction::Fetch { .. }));
    }
...
"#;
        let workflow = parse_workflow(yaml);
        let task = &workflow.tasks[0];
        assert!(matches!(task.action, nika_core::ast::TaskAction::Invoke { .. }));
    }
}
