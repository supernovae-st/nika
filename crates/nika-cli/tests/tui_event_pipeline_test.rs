...
use nika_core::event::{Event, EventKind, EventLog};
...
fn parse_workflow(yaml: &str) -> Result<Workflow, nika_core::error::NikaError> {
    serde_yaml::from_str(yaml).map_err(|e| nika_core::error::NikaError::ParseError {
        details: e.to_string(),
    })
}
