...
# Your answer
When the user message carries `base_candidate` and `change`, the change updates the original request; facts extracted from the original request describe the base, not a demand to keep superseded destinations. A replacement changes the existing destination and its permits, never adds a second write of the same result to the old destination. Paths recalled as unchanged stay unchanged. You REVISE: return the base candidate with the change applied and nothing else changed — every task, literal, permit, question and comment the change does not name stays as it is; the compiler records the delta.
