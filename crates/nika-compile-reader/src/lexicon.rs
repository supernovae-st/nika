...
use super::{gates, hot, objects, words};
pub use cues::settle_retrieval;
pub(crate) use cues::{ARTICLES, OBJECT_CONNECTORS};
use cues::{
    CADENCE_WORDS, CLOCK_SUFFIXES, CONSTRAINT_OPENERS, DEDUP_MARKERS, FINAL_GATE_MARKERS,
    FORBIDDEN_MARKERS, HEAD_FILLERS, LEADING_FILLER, NAMED_GATE_MARKERS, NEGATION_OPENERS,
    REVISION_MARKERS, SECOND_WORD_FILLERS, STOP_MARKERS, STRONG_CONNECTORS, TRIGGER_PREFIXES,
...
fn cut_head<'a>(sentence: &'a str, text: &str, reading: &mut Reading) -> (&'a str, String) {
...
mod head_tests {
    ...
    fn a_schedule_no_comma_closes_is_the_head_in_german_portuguese_and_english() {
        for (intent, head, rest) in [
            ...
            let prefix = super::TRIGGER_PREFIXES
                .iter()
