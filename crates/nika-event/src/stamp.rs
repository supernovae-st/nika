...
//! pairs · [`EventSink`] receives the stream. Home: `nika-event` since
//! P3 B7 (descended from the runtime at its 15k wall — the stamper IS
//! event machinery; `nika_runtime::stamp` re-exports, paths unchanged).
//!
...
mod tests {
    ...
    fn vec_sink_preserves_emission_order() {
        ...
        for kind in [
            crate::EventKind::WorkflowStarted,
            crate::EventKind::WorkflowCompleted,
        ] {
        ...
        assert_eq!(events[0].kind, crate::EventKind::WorkflowStarted);
        assert_eq!(events[1].kind, crate::EventKind::WorkflowCompleted);
    }
