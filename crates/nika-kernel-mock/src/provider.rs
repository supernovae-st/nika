    ...
    async fn infer_stream(&self, request: InferRequest) -> Result<InferEventStream, ProviderError> {
        ...
        for block in &response.content {
            if let ContentBlock::Text { text } = block {
                events.push(Ok(InferEvent::Delta { text: text.clone() }));
            }
        }
...
mod tests {
    ...
    async fn records_requests() {
        ...
        let _ = provider
            .infer(InferRequest::new("claude-sonnet", vec![]))
            .await;
    ...
    async fn enqueue_response_populates_queue() {
        ...
        assert_eq!(resp.content.len(), 1);
        assert!(
            matches!(&resp.content[0], ContentBlock::Text { text } if text == "custom content")
        );
    ...
    async fn enqueue_error_populates_queue() {
        let provider = MockProvider::new("test").enqueue_error(ProviderError::ModelNotFound {
            model: "gpt-9".into(),
        });
        let result = provider.infer(InferRequest::new("m", vec![])).await;
        assert!(result.is_err());
