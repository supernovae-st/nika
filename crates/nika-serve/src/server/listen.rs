...
pub(crate) fn listen_line(addr: SocketAddr) -> String {
    let hops = "GET /health · GET /v1/openapi.json (Bearer) · POST /v1/check (Bearer) · POST /v1/jobs (Bearer + Idempotency-Key · {\"workflow\": <a served name>} or the snapshot `nika check <file> --json --sdk-snapshot` prints) · PUT/GET /v1/schedules/:id (Bearer + ETag)";
...
mod tests {
    ...
    fn listen_line_names_the_authenticated_next_hops() {
        ...
        assert!(line.contains("POST /v1/check"), "{line}");
