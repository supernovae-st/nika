...
/// The last pin `execute_with_access` observed (`None` = unpinned job).
#[derive(Clone, Debug, PartialEq, Eq)]
pub(super) enum ObservedPin {
    Unpinned,
    Pinned(String),
}

/// Records the job's `--access` pin as handed to the backend.
pub(super) struct PinRecordingBackend {
    calls: AtomicUsize,
    pin: Mutex<Option<ObservedPin>>,
}

impl PinRecordingBackend {
    pub(super) fn new() -> Self {
        Self {
            calls: AtomicUsize::new(0),
            pin: Mutex::new(None),
    ...
    pub(super) fn last_pin(&self) -> Option<ObservedPin> {
        self.pin.lock().expect("pin mutex").clone()
    }
}

impl ExecutionBackend for PinRecordingBackend {
    fn execute<'a>(
        &'a self,
        _context: nika_execution::ExecutionContext<'a>,
    ) -> Pin<Box<dyn Future<Output = ExecutionOutcome> + Send + 'a>> {
    ...
    fn execute_with_access<'a>(
        ...
        self.calls.fetch_add(1, Ordering::SeqCst);
        *self.pin.lock().expect("pin mutex") = Some(match access_pin {
            Some(pin) => ObservedPin::Pinned(pin.to_owned()),
            None => ObservedPin::Unpinned,
        });
        Box::pin(async { ExecutionDisposition::Succeeded.into() })
    }
}
