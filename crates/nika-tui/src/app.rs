...
use nika_core::util::constants::{EXEC_TIMEOUT, FETCH_TIMEOUT, INFER_TIMEOUT, WORKFLOW_TIMEOUT};

use nika_core::ast::schema_validator::WorkflowSchemaValidator;
use nika_core::ast::{AgentParams, McpConfigInline, Workflow};
use nika_core::error::{NikaError, Result};
use nika_core::event::{Event as NikaEvent, EventKind, EventLog};
use nika_mcp::McpClient;
use nika_mcp::McpConfig;
use nika_provider::rig::{RigProvider, StreamChunk};
use nika_runtime::{RigAgentLoop, RigAgentStatus, Runner};
...
use nika_core::config::mask_api_key;
...
impl App {
    ...
    fn poll_runtime_events(&mut self) {
        ...
        let mut events: Vec<nika_core::event::Event> = Vec::new();
        ...
        match action {
            ...
            McpAction::Toggle(server) => {
                ...
                if is_selected {
                    ...
                    // Add to selection
                    use crate::widgets::McpServerInfo;
                    self.chat_view
                        .session_context
                        .mcp_servers
...
fn render_chat_overlay(frame: &mut Frame, state: &TuiState, theme: &Theme, area: Rect) {
    ...
    for msg in &chat.messages {
        let (prefix, style) = match msg.role {
            crate::state::ChatOverlayMessageRole::User => (
                "[You]",
                Style::default()
                    .fg(theme.trait_retrieved) // Cyan
                    .add_modifier(Modifier::BOLD),
            ),
            crate::state::ChatOverlayMessageRole::Nika => (
                "[AI]",
                Style::default()
                    .fg(theme.status_success) // Green
                    .add_modifier(Modifier::BOLD),
            ),
            crate::state::ChatOverlayMessageRole::System => (
                "[System]",
                Style::default()
                    .fg(theme.status_running) // Yellow/Amber
                    .add_modifier(Modifier::BOLD),
            ),
            crate::state::ChatOverlayMessageRole::Tool => (
                "[Tool]",
                Style::default()
