//! nika-tui — Terminal UI for Nika workflow engine
//!
...
#[cfg(feature = "tui")]
pub use keybindings::{KeyCategory, Keybinding, format_key, keybindings_for_context};
#[cfg(feature = "tui")]
...
#[cfg(feature = "tui")]
pub async fn run_tui(workflow_path: &std::path::Path) -> nika_core::error::Result<()> {
    use nika_core::ast::Workflow;
    use nika_core::event::EventLog;
    ...
    let yaml_content = tokio::fs::read_to_string(workflow_path)
        .await
        .map_err(|e| nika_core::error::NikaError::WorkflowNotFound {
    ...
    let workflow: Workflow = serde_yaml::from_str(&yaml_content).map_err(|e| {
        let line_info = e
            ...
            .unwrap_or_default();
        nika_core::error::NikaError::ParseError {
...
#[cfg(feature = "tui")]
pub async fn run_tui_standalone() -> nika_core::error::Result<()> {
    // Install panic hook for terminal recovery
    install_panic_hook();
...
#[cfg(feature = "tui")]
pub async fn run_tui_studio(workflow: Option<std::path::PathBuf>) -> nika_core::error::Result<()> {
    use views::TuiView;
...
#[cfg(not(feature = "tui"))]
pub async fn run_tui(_workflow_path: &std::path::Path) -> nika_core::error::Result<()> {
    Err(nika_core::error::NikaError::ValidationError {
...
#[cfg(not(feature = "tui"))]
pub async fn run_tui_studio(_workflow: Option<std::path::PathBuf>) -> nika_core::error::Result<()> {
    Err(nika_core::error::NikaError::ValidationError {
        reason: "TUI feature not enabled. Rebuild with --features tui".to_string(),
    })
}
