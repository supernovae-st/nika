...
use nika_core::config::NikaConfig;
use nika_core::event::{ContextSource, EventKind, ExcludedItem};
        ...
        match field {
            SettingsField::AnthropicKey => {
                if let Some(ref key) = self.config.api_keys.anthropic {
                    (true, nika_core::config::mask_api_key(key, 12))
                } else {
            ...
            SettingsField::OpenAiKey => {
                if let Some(ref key) = self.config.api_keys.openai {
                    (true, nika_core::config::mask_api_key(key, 10))
                } else {
                    (false, "Not set".to_string())
                }
            }
                ...
                for task_id in &self.task_order {
                    if let Some(task) = self.tasks.get(task_id) {
                        let status = match task.status {
                            crate::theme::TaskStatus::Pending => "○",
                            crate::theme::TaskStatus::Running => "◐",
                            crate::theme::TaskStatus::Success => "✓",
                            crate::theme::TaskStatus::Failed => "✗",
                            crate::theme::TaskStatus::Paused => "⏸",
                        };
