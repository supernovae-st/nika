...
supersedes: []
superseded_by: []
related: ["ADR-006", "ADR-007", "ADR-014", "ADR-016", "ADR-028", "ADR-032", "ADR-034", "ADR-035", "ADR-080", "ADR-107"]
...
- `crates/nika-kernel-core/src/cancel.rs` — `CancelCtx` (real impl).
- `crates/nika-kernel/src/observability.rs` — `ObservabilitySink` (stub).
- `crates/nika-kernel-plugin/src/wasm.rs` — `WasmPluginHost`, `PluginFs`,
  `PluginHttp`, `WasmPluginError` (stubs).
- `crates/nika-kernel-plugin/src/sandbox.rs` — `Sandbox`, `Capability`,
  `SandboxError` (stubs).
...
- `crates/nika-kernel-plugin/src/wasm.rs:11-22` — `WasmPluginHost` trait.
- `crates/nika-kernel-plugin/src/wasm.rs:24-39` — `PluginFs`, `PluginHttp`.
- `crates/nika-kernel-plugin/src/wasm.rs:50-83` — `WasmPluginError` enum.
- `crates/nika-kernel-plugin/src/sandbox.rs:18-26` — `Sandbox` trait.
- `crates/nika-kernel-plugin/src/sandbox.rs:28-54` — `Capability` enum.
- `crates/nika-kernel-plugin/src/sandbox.rs:78-99` — `SandboxError`.
