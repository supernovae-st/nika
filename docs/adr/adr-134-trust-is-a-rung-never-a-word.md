...
3. **Every wire row says it.** `lane_rows` adds `trust` beside `billing` (a refused lane says `trust: null`: no path serves, its rejected candidates carry their own witnesses; a consumer never reads a missing key as a rung) — `check --json` · `run --dry-run --json` · the trace boot manifest · the resident's check summary — and the two human lines (`check`'s access rung · `run`'s announce) print the rung word in the same parenthesis as the class and the billing.
