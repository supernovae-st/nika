...
|---|---|---|---|---|
| L0 | Pure types, lookup tables, sync-only APIs (live counts: refresh-status block) | none | (leaf) | `nika-types`, `nika-error`, `nika-catalog`, `nika-catalog-codegen`, `nika-schema`, `nika-source`, `nika-vocab`, `nika-graph`, `nika-event`, `nika-pack`, `nika-migrate` · *planned* · `nika-binding`, `nika-transform`, `nika-pck-manifest` |
