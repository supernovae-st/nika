...
| L4 | Interfaces — transport/UI surface, libraries only | via L3 | L0..L3 · plus **lateral L4→L4** edges (`check-layering.sh` refuses only an UPWARD rank; a same-rank edge is admitted so one interface crate can reuse another's pure seam instead of cloning it — `nika-mcp → nika-onboard` for the ONE routing door, `nika-mcp → nika-cli-host` for the ONE `check --fix` ladder (#1270), `nika-cli-host → nika-onboard` for the ONE typed Compile core and shared read-only Foundry knowledge door (#1671), `nika-serve → nika-onboard` for the same core and its ONE machine document behind `POST /v1/compile` (#1670 · a transport, never a second compiler), `nika-serve → nika-cli-host` (default features off) for the ONE authoring configuration parser and Foundry knowledge reader behind the operator's native authoring seat (S06 · compile generation 2 · `nika-cli-host` never reaches `nika-serve`); the edge must stay acyclic and the reused member must never depend back) | `nika-cli`, `nika-arm` (descriptor-rooted ARM custody + the one injected firer shared by CLI and Serve), `nika-cli-host` (size-cap member of the nika-cli unit · D-2026-07-09-N1 · ADR-110), `nika-trace` (size-cap member of the nika-cli unit · the flight-recorder reader · D-2026-07-09-N1 · 2026-08-11), `nika-dap`, `nika-display`, `nika-onboard`, `nika-compile` (size-cap member of the nika-onboard unit · the stateless Compile core · D-2026-07-09-N1 · ADR-137 · lateral `nika-onboard → nika-compile`, never back), `nika-compile-cognition` (size-cap member of the same admitted onboarding unit · seat orchestration above the deterministic core · D-2026-07-09-N1 · ADR-140 · production edges to `nika-compile`, `nika-compile-reader` and `nika-compile-fidelity`, never back; core suites use a development edge), `nika-compile-reader` (size-cap member of the same unit · the frozen deterministic reader and the typed plan it produces · D-2026-07-09-N1 · ADR-138 · lateral `nika-compile → nika-compile-reader`, never back), `nika-compile-fidelity` (size-cap member of the same unit · the laws a candidate document is judged by, the seat's sketch and the plan a candidate states · D-2026-07-09-N1 · ADR-141 · lateral `nika-compile → nika-compile-fidelity → nika-compile-reader`, never back), `nika-models`, `nika-daemon`, `nika-serve`, `nika-mcp`, `nika-lsp`, `nika-sdk`, `nika-catalog-verify`, `nika-check-wasm` (WIP · ADR-107), `nika-session` (the native session · a host runtime over the installed engine · lateral `nika-session → nika-cli-host` for the ONE probe and the ONE oracle facade · lateral `nika-session → nika-trace` for the run facts behind its result, gate and `/proof` views, never back · ADR-125) |
