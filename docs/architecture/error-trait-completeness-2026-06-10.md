...
| `JobStoreError` (nika-serve) | **transport-surface** · L4 remote-job state-plane refusal. It never becomes a workflow task error; the future HTTP adapter maps it to bounded transport responses. No workflow NIKA range is owed before that transport contract exists (crate spec §error boundary). |
