...
| 3 | **Episodes** | Compressed representation at completion boundary (not mid-stream) | Only important results retained |
| 4 | **Thread Weaving** | Orchestrator: dispatch threads → collect episodes → synthesize → dispatch | Implicit adaptive decomposition |
| 5 | **Strategy/Tactics** | Strategy = open-ended planning. Tactics = learned action sequences | AlphaZero mapping (value + policy networks)[^2] |
| 6 | **Knowledge Overhang** | Models have knowledge they can't access without scaffolding | Episodes provide the scaffolding |
| 7 | **Composability** | Episodes flow between threads as handoff boundary | Cross-model composition |
| 8 | **OS Framing** | Orchestrator = kernel, Threads = processes, Episodes = return values | Karpathy's LLM OS framing |

</details>

> [!NOTE]
> **Slate's model configuration** — Slate supports cross-model composition (e.g., Sonnet + Codex for different cognitive tasks)[^1]. The "4 named slots" (main/tactical/search/reasoning) is our design, inspired by this capability.
...
> [!TIP]
> **What's missing is NOT the kernel** — it's 4 kernel upgrades: episode compression, dynamic process creation (strategy), memory budgets, and model routing. The kernel itself (`Runner` + `TaskExecutor` + `DataStore`) already works.
...
flowchart TB
    ...
    subgraph L2["Layer 2 — Episode Engine"]
        EE["episode:\ncompression at completion boundary"]
    end

    subgraph L3["Layer 3 — Strategy Orchestration"]
        SO["orchestration: strategy\ndynamic tactic dispatch"]
    end
...
model_slots:
  ...
  tactical:  { provider: groq,     model: llama-3.3-70b-versatile }
...
See [05-evolution-roadmap.md § P-EPISODE](./05-evolution-roadmap.md#p-episode-episode-engine) for full design.
...
The core upgrade. See [05-evolution-roadmap.md § P-STRATEGY](./05-evolution-roadmap.md#p-strategy-strategy-orchestration) for full design.

```mermaid
sequenceDiagram
    participant S as 🎯 Strategy LLM
    participant T as ⚡ Tactic Templates

    loop Until DONE or max_rounds
        S->>S: Review accumulated episodes
        S->>T: Dispatch tactic(s) with params
        T-->>S: Episode(s) with confidence
        S->>S: Synthesize episodes
...
flowchart LR
    ...
    subgraph NIKA_HAS["Nika Has (after integration)"]
        N1["YAML tactic templates"]
        N2["NovaNet graph memory"]
        N3["34 events + NDJSON"]
        N4["Episode budget + tokens"]
        N5["200+ locales"]
    end
...
| Feedback | Per-step | End | End | None | Per-task | Per-step | Per-episode | **Per-episode** |
| Context isolation | None | None | Partial | None | Full | None | Per-thread | **Per-task** |
| Compression | Compact | Compact | None | None | Compress | Compact | Episode | **Episode+NovaNet** |
| Parallelism | None | None | None | None | Multi-agent | None | Native | **for_each+strategy** |
...
| **Cost control** | None | None | None | None | None | None | None | **Episode budget** |
...
<details>
<summary>📋 Full Strategy Workflow — Landing Page Generation</summary>
...
The old P3 ConfidenceRouter is absorbed into episodes. Confidence is a property, not a system:
...
flowchart LR
    ...
    subgraph NEW["New: Episode Confidence"]
        N1["Task"] --> N2["Episode\nconfidence: 0.6"]
        N2 --> N3["Strategy LLM\nsees low confidence"]
...
> [!TIP]
> The strategy LLM has **full context** to decide how to handle low confidence. A rigid router has fixed rules. This is simpler AND more powerful.
...
mindmap
    root((Nika Evolution))
        Slate's Insights
            Threads → Tasks
            Episodes → Compressed results
            Weaving → Strategy orchestration
...
[^2]: McGrath et al., "Acquisition of Chess Knowledge in AlphaZero" — [PNAS 2022](https://www.pnas.org/doi/10.1073/pnas.2206625119). Strategy/tactics separation cited in Slate blog.
