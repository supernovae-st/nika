...
┣━━━━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃ ⚡ LIVE │ hero [Claude/main+🧠] "Créez des codes QR intelligents qui..." │ 5.2K in │ 890 out │ struct: ⏳  ┃
┃ 💰 $0.037 │ 🧠 reasoning ◉ main ◉ search ◉ tactical ○ │ 🔗 NovaNet: qr-code-ai/fr-FR │ EP: 4+2R    ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
```
...
### Panel 1: Strategy Commander (top-left)
...
| Strategy thinking | Live preview of strategist's reasoning (truncated) | Streaming |
| Next actions | Queue of planned tasks (from strategy) | Per decision |

**Color coding:**
- Round ring: teal gradient filling
- Quality: green ≥ 0.85, orange 0.70–0.84, red < 0.70
- Budget: teal → orange at 80% → red at 95%
- Model dots: purple (reasoning), blue (main), orange (search), gray (tactical)
...
| Token count | "500 tok" badge on completed nodes (episode size) | On completion |
...
### Panel 4: Episodes Timeline (bottom-left)

The "memory bank". Shows the compressed episode history.

| Episode type | Visual | Meaning |
|-------------|--------|---------|
...
- Date (for recalled episodes)
- Entity link (for persisted episodes)

**Bottom stats:**
- Total working memory: sum of all active episode tokens
...
| **episode: config** | compress, max_tokens, retain fields, persist target |
...
| Token usage | Total in/out/thinking/episode tokens |
| Episode efficiency | Savings percentage (compression ratio) |
...
| `↑↓` | Navigate within panel (episodes, decisions, tasks) |
| `Enter` | Expand selected item (episode detail, task detail) |
| `Space` | Pause/resume strategy (manual override) |
| `d` | Toggle DAG mode (tree/flat/radial) |
| `e` | Toggle episode panel (collapsed/expanded) |
...
| `Ctrl+P` | Pause strategy and enter manual mode |
...
║  #7c3aed  Purple   Reasoning model, strategy decisions, thinking           ║
║  #2563eb  Blue     Main model, primary execution, MCP protocol             ║
║  #d97706  Orange   Search model, warnings, alerts, tactical                ║
║  #b58900  Gold     Episodes, compressed data, memory packets               ║
...
║  ── Model Slot Colors ──                                                   ║
║                                                                            ║
║  reasoning  #7c3aed  Purple  (deep thought, extended thinking)             ║
║  main       #2563eb  Blue    (primary execution, quality output)           ║
║  search     #d97706  Orange  (fast, cheap, information gathering)          ║
║  tactical   #586e75  Gray    (utility, formatting, simple tasks)           ║
║                                                                            ║
║  ── Episode Colors ──                                                      ║
║                                                                            ║
║  New episode:      #b58900  Gold border                                    ║
...
orchestrator called "Nika v0.30 — Strategy Runner". This is the user's view
...
PANEL 1 — STRATEGY COMMANDER (top-left, 15% width):
- Header: "🎯 STRATEGY COMMANDER" with teal accent line
...
PANEL 4 — EPISODES TIMELINE (bottom-left, 15% width):
- Header: "📦 EPISODES" with gold accent
- Stack of 6 episode cards (scrollable):
...
- Task info: verb ⚡ infer, model Claude (main slot + 🧠 thinking)
...
- Header: "📈 METRICS"
- Cost table: reasoning $0.018, main $0.012, search $0.004, tactical $0.003
  TOTAL: $0.037 (in gold)
- Token table: In 18,420 | Out 4,890 | Thinking 8,200 | Episodes 2,200
  "Saved by episodes: 74%"
...
- Line 1: "⚡ LIVE │ hero [Claude/main+🧠] 'Créez des codes QR
  intelligents qui...' │ 5.2K in │ 890 out │ struct: ⏳"
- Line 2: "💰 $0.037 │ 🧠 ◉ reasoning ◉ main ◉ search ○ tactical │
...
- The gold episode cards in the left panel should feel like "memory packets"
- The teal NovaNet panel should feel like an "intelligence briefing"
- The purple strategy thinking should feel like "the AI explaining itself"
...
2. TOP-RIGHT: MODEL_ROUTER
   ...
   Label: "4-slot routing: reasoning/main/search/tactical"
...
6. LEFT: INTROSPECTION_TOOLS
   ...
   Label: "6 runtime tools: episodes, threads, cost, dag, status, strategy"
   Sub-elements: 6 small tool icons

BETWEEN ORBITAL MODULES:
Golden hexagonal data packets (episodes) flowing along the orbital path.
Each packet is a small hexagon with a token count inside ("400 tok").
The flow direction is clockwise, showing data lifecycle:
BIND (with:) → EXECUTE → VALIDATE (structured:) → COMPRESS (episode:) →
...
- OUTPUT section (bottom): structured: validation, episode: compression
...
- "episode: prevents the dumb zone" (near COMPRESS section)
- "NovaNet remembers across sessions" (near roots)
- "Strategy = Slate's thread weaving" (near STRATEGY module)
...
- Purple (#7c3aed): Strategy, reasoning, thinking
- Blue (#2563eb): Main execution, context management
- Orange (#d97706): Search, tactical, warnings
- Gold (#b58900): Episodes, data packets, memory
...
- The butterfly's wings are composed of:
  LEFT WING — "The Body" (Execution):
    ...
    - Episode data packets (golden hexagons) flowing through
...
- Bottom: 6 feature badges in a horizontal row:
  "Model Slots" | "Episodes" | "Strategy" | "Context Budget" | "Memory" | "Introspect"
  Each with its icon and accent color

FLOATING DATA ELEMENTS around the butterfly:
- Golden episode hexagons drifting like fireflies
...
- Version: "v0.30 • YAML-first • Knowledge-aware • Strategy-driven"

COLOR TREATMENT:
- Predominantly dark with selective color accents
- Teal (#0d9488) for NovaNet/knowledge elements (right wing)
- Purple (#7c3aed) for strategy/reasoning (top of butterfly)
- Blue (#2563eb) for execution paths (left wing)
- Gold (#b58900) for episodes (scattered throughout)
