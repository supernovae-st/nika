...
Nika is designed as a terminal-first tool. Simple workflows run headlessly via `nika run workflow.nika.yaml`. Complex interactions happen in a ratatui-based TUI with three views: Studio (editor + DAG preview), Command (execution monitoring and chat), and Control (configuration). There is no web UI, no Electron wrapper. The terminal is the interface.
...
The Model Context Protocol is Nika's extensibility mechanism. Any MCP server can be declared in the workflow's `mcp:` block and its tools become available to `invoke:` and `agent:` verbs. Nika also ships 24 builtin tools under the `nika:*` namespace.
...
- **Not a chatbot framework.** Nika's `agent:` verb enables agent loops, but Nika itself is not a chat application. The TUI's Command view provides chat mode as one of three views.
...
| MCP aliases | 100 |
| Builtin tools | 24 (nika:* namespace) |
