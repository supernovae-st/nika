...
| 5 MUTATION ≥90 | ✅ **BUDGET mode** `GATE5-EXEMPT: 35` (sanctioned · cf `nika-screen` 15 · `nika-browser` 9). The killable survivors are dead post-hardening (the jq error-render fns + the permits access-category projection assert message CONTENT now · `missed` 30→21). The residual ~32 survivors (grounded in the least-contended run · 21 `missed` + 11 `timeout`) are: **equivalent mutants** (base64 disjoint-bit `\|`↔`^` · `\|`==`^` when the operands share no bits · un-killable), **hard byte-scanner edge ops** (charset detection · `is_not_a_directory` non-ENOTDIR), and **I/O-bound `timeout` mutants** the harness cannot distinguish from real hangs. The clean FLOOR is ~92% (NOT the contention-inflated 86.8%) — a future low-load re-run RETIRES this budget. A few residual mutants ARE in the permits/net security files but are **equivalent / mock-undistinguishable** (defensive guards whose condition never diverges for a real path · the `unbounded()`↔`Default` coincidence · the `.get()/.post()` dispatch arm a test double cannot tell apart — the meaningful `request.method` mapping IS pinned · all in-source documented) — **none an exploitable bypass**; the rust-security review verified the boundary **SOUND**. <!-- GATE5-EXEMPT: 35 --> |
