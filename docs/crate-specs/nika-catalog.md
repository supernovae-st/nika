...
## Exact inference admission tariffs

`data/inference-admission.toml` is a separately dated official-source catalog,
generated by `nika-catalog-codegen`, never a rewrite of models.dev provenance.
`InferenceTariff::new(provider, model, endpoint)` only accepts exact bindings;
there is no substring lookup, gateway nickname inference or free unpriced row.
