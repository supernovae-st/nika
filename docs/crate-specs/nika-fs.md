...
| LOC budget | ≤400 src (actual ~342 · doc-comment heavy: per-method CANCEL SAFETY + replace-semantics contracts) |
...
1. **Atomic write** — brouillon used bare `tokio::fs::write` (partial
   writes observable). Diamond writes to `.{name}.{pid}.{counter}.nika-tmp`
...
| 3 IMPL | ✅ | ~342 LOC src · zero unwrap/expect in src (grep-verified 0) |
