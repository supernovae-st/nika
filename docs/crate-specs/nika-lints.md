...
| Layer | **L0** — pure, zero I/O, zero async; deps `nika-schema` (RawWorkflow · spans · the shared native-first classifier) + `serde_json` (rule-table payload shapes). |
| Design | The spec-normative **advisory lint passes** — warnings, never errors (spec `03-dag.md` §One obvious way · « the discouraged forms are legal · just not canonical »). Three rule sets: `one_obvious_way` (the 7 preference rules the spec marks « normative for linters »), `native_first` (verb-choice advisories · exec with a probable native path · classifier shared with the check ladder via `nika_schema::check::native_first::classify`), `arg_injection` (argument-injection advisories for the exec array form · spec `02-verbs.md` §exec Security). |
...
| Deps | `nika-schema`, `serde_json`. dev: `serde`. |
