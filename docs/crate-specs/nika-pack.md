...
| LOC budget | workspace crate and file ratchets apply |
...
JSON Schema, canon registry, the canonical examples, the instantiable
templates and the manifest that hashes them (nika-spec README §The examples
pack). `nika try` / `nika compile` / `nika spec` read this crate — offline, version-locked, verifiable.
...
pub fn template_example(name: &str) -> Option<&'static str>; // paired filled lesson
...
- `every_manifest_entry_resolves_and_hashes_clean` — re-hashes the LEAN text
  of every manifest workflow and template against the manifest's
...
- `surface_counts_hold` — nonempty templates, examples and doc pages ·
