...
pre_spawn = validator["create_run_temp_root"]()
unexpected = pathlib.Path(RUN_TEMP) / "unexpected"
unexpected.write_text("keep")
expect_red(lambda: validator["cleanup_run_temp_root"](pre_spawn))
validator["require"](
    unexpected.read_text() == "keep",
    "cleanup deleted unexpected temporary-root content",
)
unexpected.unlink()
validator["cleanup_run_temp_root"](pre_spawn)
...
validator["require"](failures == 29, "one of 29 negative fixtures survived")
print("OK: validator green fixtures passed; 29/29 negative mutations rejected")
