...
exclusions = json.loads(json.dumps(validator["EQUIVALENT_EXCLUSIONS"]))
start = {
    "equivalent_exclusions": exclusions,
    "prior_anomalies": [{"current_name": "m:caught"}],
    "guard_mutants": [],
}
controls = validator["SAME_SPAN_CONTROLS"]
census = validator["APPROVED_EQUIVALENT_NAMES"] + controls + ["m:caught", "m:unviable"]
mutants = [{"name": name} for name in controls + ["m:caught", "m:unviable"]]
outcomes = {
    "total_mutants": 4,
    "caught": 3,
    "unviable": 1,
    ...
    "outcomes": [
        *[
            {"scenario": {"Mutant": {"name": name}}, "summary": "CaughtMutant"}
...
expect_red(
    lambda: validator["validate_equivalent_exclusions"](exclusions[:-1], census)
)
expect_red(
    lambda: validator["validate_equivalent_exclusions"](
        exclusions + [json.loads(json.dumps(exclusions[-1]))], census
    )
)
mislocated = json.loads(json.dumps(exclusions))
mislocated[1]["name"] = mislocated[1]["name"].replace(":1108:", ":1110:")
mislocated[1]["anchored_regex"] = validator["anchored_literal_regex"](
    mislocated[1]["name"]
)
expect_red(lambda: validator["validate_equivalent_exclusions"](mislocated, census))
broad = json.loads(json.dumps(exclusions))
broad[1]["anchored_regex"] = ".*LifecycleValidator::accept.*"
expect_red(lambda: validator["validate_equivalent_exclusions"](broad, census))
synthetic = json.loads(json.dumps(exclusions))
synthetic[1]["proof"]["method"] = "synthetic impossible-state tests as score"
expect_red(lambda: validator["validate_equivalent_exclusions"](synthetic, census))
run_argv = ["cargo", "mutants"]
for exclusion in exclusions:
    run_argv.extend(["-E", exclusion["anchored_regex"]])
validator["validate_exclusion_argv"](exclusions, run_argv)
expect_red(lambda: validator["validate_exclusion_argv"](exclusions, run_argv[:-2]))
expect_red(
    lambda: validator["validate_exclusion_argv"](
        exclusions, [*run_argv, "-E", exclusions[0]["anchored_regex"]]
    )
)
broad_argv = list(run_argv)
broad_argv[3] = ".*LifecycleValidator::accept.*"
expect_red(lambda: validator["validate_exclusion_argv"](exclusions, broad_argv))
...
def clear_fault_roots() -> None:
    ...
    if os.path.lexists(RUN_LEASE):
        if RUN_LEASE.is_symlink():
            RUN_LEASE.unlink()
        else:
            descriptor, _record = validator["open_lease_candidate"]()
            try:
                validator["acquire_lease"](descriptor)
                validator["unlink_locked_lease"](
                    descriptor, "selftest-lease-cleanup"
                )
            finally:
                os.close(descriptor)
...
def create_stale_lease(receipt: str = "fixture-receipt") -> dict[str, Any]:
    descriptor, record = validator["create_execution_lease"](receipt)
    os.close(descriptor)
    return record
...
for fault_stage in fault_stages:
    ...
    validator["require"](
        all(
            ...
            for path in (RUN_OUTPUT, RUN_TARGET, RUN_TEMP, RUN_LEASE)
        ),
...
validator["require"](not os.path.lexists(RUN_LEASE), "settled lease was not retired")
clear_fault_roots()
...
expect_red(classify_fixture)
validator["require"](
    os.path.lexists(RUN_OUTPUT) and not os.path.lexists(RUN_LEASE),
    "scratch without a lease was not preserved",
)
clear_fault_roots()

create_fault_roots("output")
RUN_LEASE.write_text("{")
os.chmod(RUN_LEASE, 0o600)
expect_red(classify_fixture)
validator["require"](
    os.path.lexists(RUN_OUTPUT) and os.path.lexists(RUN_LEASE),
    "invalid lease and scratch were not preserved",
)
clear_fault_roots()

create_fault_roots("output")
RUN_LEASE.write_bytes(validator["lease_payload"]("wrong-receipt"))
os.chmod(RUN_LEASE, 0o600)
expect_red(classify_fixture)
validator["require"](
    os.path.lexists(RUN_OUTPUT) and os.path.lexists(RUN_LEASE),
    "wrong lease identity and scratch were not preserved",
)
clear_fault_roots()

with tempfile.TemporaryDirectory(dir="/tmp") as lease_external:
    survivor = pathlib.Path(lease_external) / "lease"
    survivor.write_text("external")
    create_fault_roots("output")
    RUN_LEASE.symlink_to(survivor)
    expect_red(classify_fixture)
    validator["require"](
        RUN_LEASE.is_symlink() and survivor.read_text() == "external",
        "lease symlink was deleted or followed",
    )
    clear_fault_roots()

lease_globals = validator["unlink_locked_lease"].__globals__
original_lease_parent_fsync = lease_globals["fsync_scratch_parent"]
recovery_boundaries: list[tuple[str, bool, bool]] = []


def record_recovery_boundary(stage: str) -> dict[str, Any]:
    recovery_boundaries.append(
        (
            stage,
            all(not os.path.lexists(path) for path in (RUN_OUTPUT, RUN_TARGET, RUN_TEMP)),
            os.path.lexists(RUN_LEASE),
        )
    )
    return original_lease_parent_fsync(stage)


create_stale_lease()
create_fault_roots("output", "target", "temp")
lease_globals["fsync_scratch_parent"] = record_recovery_boundary
try:
    ordered_recovery = classify_fixture()
finally:
    lease_globals["fsync_scratch_parent"] = original_lease_parent_fsync
validator["require"](
    ordered_recovery["action"] == "recovered"
    and recovery_boundaries
    == [
        ("startup-recovery-removal-authority-before-lease", True, True),
        ("startup-recovery-removal-lease-unlinked", True, False),
    ],
    f"lease recovery durability order drift: {recovery_boundaries}",
)

first_barrier_failed = False


def fail_first_recovery_barrier(stage: str) -> dict[str, Any]:
    global first_barrier_failed
    if stage == "startup-recovery-removal-authority-before-lease":
        first_barrier_failed = True
        raise OSError("injected root-removal parent fsync failure")
    return original_lease_parent_fsync(stage)


create_stale_lease()
create_fault_roots("output", "target", "temp")
lease_globals["fsync_scratch_parent"] = fail_first_recovery_barrier
try:
    expect_red(classify_fixture)
finally:
    lease_globals["fsync_scratch_parent"] = original_lease_parent_fsync
validator["require"](
    first_barrier_failed
    and all(not os.path.lexists(path) for path in (RUN_OUTPUT, RUN_TARGET, RUN_TEMP))
    and os.path.lexists(RUN_LEASE),
    "first barrier failure lost the recovery lease or retained roots",
)
validator["require"](
    classify_fixture()["action"] == "recovered",
    "first barrier failure was not retryable",
)

second_barrier_failed = False


def fail_second_recovery_barrier(stage: str) -> dict[str, Any]:
    global second_barrier_failed
    if stage == "startup-recovery-removal-lease-unlinked":
        second_barrier_failed = True
        raise OSError("injected lease-unlink parent fsync failure")
    return original_lease_parent_fsync(stage)


create_stale_lease()
create_fault_roots("output", "target", "temp")
lease_globals["fsync_scratch_parent"] = fail_second_recovery_barrier
try:
    expect_red(classify_fixture)
finally:
    lease_globals["fsync_scratch_parent"] = original_lease_parent_fsync
validator["require"](
    second_barrier_failed
    and all(not os.path.lexists(path) for path in (RUN_OUTPUT, RUN_TARGET, RUN_TEMP))
    and not os.path.lexists(RUN_LEASE),
    "second barrier failure left roots or an ambiguous lease state",
)
validator["require"](
    classify_fixture()["action"] == "clean",
    "second barrier failure was not retryable",
)
create_stale_lease()
validator["require"](
    classify_fixture()["action"] == "recovered",
    "a crash-resurrected lease with durable root absence was not recoverable",
)

with tempfile.TemporaryDirectory(dir="/tmp") as rust_lease_tmp:
    rust_root = pathlib.Path(rust_lease_tmp)
    rust_source = rust_root / "lease_holder.rs"
    rust_binary = rust_root / "lease-holder"
    leader_file = rust_root / "leader.pid"
    grandchild_file = rust_root / "grandchild.pid"
    ready_file = rust_root / "supervisor.ready"
    rust_source.write_text(
        """
use std::{env, fs, process::{self, Command}, thread, time::Duration};

fn main() {
    let args: Vec<String> = env::args().collect();
    if args[1] == "grandchild" {
        fs::write(&args[2], process::id().to_string()).unwrap();
        loop { thread::sleep(Duration::from_secs(60)); }
    }
    fs::write(&args[1], process::id().to_string()).unwrap();
...
""".strip()
    ...
    validator["require"](rustc is not None, "rustc is required for lease inheritance")
    subprocess.run(
        [rustc, str(rust_source), "-o", str(rust_binary)],
        check=True,
        capture_output=True,
        text=True,
    )
    create_stale_lease()
    create_fault_roots("output", "target", "temp")
    survivor = rust_root / "survivor"
    survivor.write_text("keep")
    (pathlib.Path(RUN_OUTPUT) / "outside-link").symlink_to(survivor)
    supervisor_code = (
        "import fcntl,os,pathlib,subprocess,time;"
        f"fd=os.open({str(RUN_LEASE)!r},os.O_RDWR);"
        "fcntl.flock(fd,fcntl.LOCK_EX);os.set_inheritable(fd,True);"
        f"p=subprocess.Popen([{str(rust_binary)!r},{str(leader_file)!r},"
        f"{str(grandchild_file)!r}],pass_fds=(fd,),start_new_session=True);"
        f"pathlib.Path({str(ready_file)!r}).write_text(str(p.pid));"
        "time.sleep(60)"
    )
    ...
    try:
        ...
        while (
            ...
            or not leader_file.exists()
            or not grandchild_file.exists()
        ...
        validator["require"](
            ...
            "Rust lease holder did not publish its process tree",
        )
        ...
        validator["require"](
            ...
            "busy lease did not preserve scratch after supervisor death",
        )
        ...
        while recovered is None and time.monotonic() < deadline:
            try:
                ...
                time.sleep(0.05)
        validator["require"](
            recovered is not None and recovered["action"] == "recovered",
            "released Rust process tree did not make the lease stale",
        )
        validator["require"](
            survivor.read_text() == "keep",
            "stale lease cleanup followed an external symlink",
        )
        validator["require"](
            classify_fixture()["action"] == "clean",
            "released lease did not become retryable",
        )
    finally:
        ...
        if leader_pid is not None:
            validator["signal_process_group"](leader_pid, signal.SIGKILL)
...
expect_red(
    lambda: validator["require_run_identity"](
        RUN_ID, "/tmp/wrong", RUN_TARGET, RUN_TEMP, str(RUN_LEASE)
    )
)
...
validator["require"](failures == 63, "one of 63 negative fixtures survived")
print("OK: validator green fixtures passed; 63/63 negative mutations rejected")
