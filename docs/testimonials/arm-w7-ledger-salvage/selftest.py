...
def finish_fixture_temp(guard: dict[str, Any]) -> None:
...
def minimal_raw_receipt() -> dict[str, Any]:
    return {
...
def completed_nonzero_process(guard: dict[str, Any]) -> tuple[int, dict[str, Any], int]:
    ...
    finish_fixture_temp(guard)
    validator["settle_raw_receipt"](guard, minimal_raw_receipt())
    return process
...
validator["require"](
    all(os.path.lexists(path) for path in (RUN_OUTPUT, RUN_TARGET, RAW_RUN_RECEIPT)),
...
term_mask = signal.pthread_sigmask(signal.SIG_BLOCK, set())
pre_settlement_pending = set()


def term_before_receipt(guard: dict[str, Any]) -> None:
    create_settlement_roots()
...
expect_red(lambda: validator["guarded_signal_temp_root"](term_before_receipt))
validator["require"](
    signal.SIGTERM in pre_settlement_pending,
    "pre-settlement TERM was not pending",
)
validator["require"](
    all(not os.path.lexists(path) for path in (RUN_OUTPUT, RUN_TARGET, RUN_TEMP)),
    "pre-settlement TERM left a scratch root",
)
validator["require"](signal.getsignal(signal.SIGTERM) == term_handler, "TERM handler drift")
validator["require"](
    signal.pthread_sigmask(signal.SIG_BLOCK, set()) == term_mask,
    "TERM mask drift",
)
...
def term_after_receipt(guard: dict[str, Any]) -> None:
    create_settlement_roots()
    fixture_process(guard)
    finish_fixture_temp(guard)
    ...
    validator["settle_raw_receipt"](
        guard, minimal_raw_receipt(), after_fsync=queue_term
    )


expect_red(lambda: validator["guarded_signal_temp_root"](term_after_receipt))
validator["require"](
    signal.SIGTERM in post_settlement_pending,
    "post-settlement TERM was not pending",
)
validator["require"](not os.path.lexists(RUN_TEMP), "settled TERM left temp root")
validator["require"](
    all(os.path.lexists(path) for path in (RUN_OUTPUT, RUN_TARGET, RAW_RUN_RECEIPT)),
    "settled TERM lost resumable evidence",
)
validator["load_raw_run_receipt"](pathlib.Path(RUN_OUTPUT), "fixture-receipt")
validator["cleanup_interrupted_scratch"]()
...
fsync_globals = validator["atomic_json_write"].__globals__
original_fsync = fsync_globals["os"].fsync
...
fsync_globals["os"].fsync = injected_fsync_failure
try:
    expect_red(
    ...
    fsync_globals["os"].fsync = original_fsync
validator["require"](
    all(not os.path.lexists(path) for path in (RUN_OUTPUT, RUN_TARGET, RUN_TEMP)),
...
with tempfile.TemporaryDirectory(dir="/tmp") as tmp:
    ...
    try:
        ...
        validator["require"](
            not any(process_exists(pid) for pid in process_ids.values()),
            "dual-signal handoff left a descendant",
        )
        validator["require"](
            validator["HANDOFF_SIGNALS"].issubset(queued),
            "SIGINT and SIGTERM were not both pending before handoff",
        )
...
payload = {{"int_blocked": signal.SIGINT in blocked, "term_blocked": signal.SIGTERM in blocked, "terminated": False}}
...
"""
    ...
    validator["require"](
        payload == {"int_blocked": False, "term_blocked": False, "terminated": True},
...
validator["require"](failures == 38, "one of 38 negative fixtures survived")
print("OK: validator green fixtures passed; 38/38 negative mutations rejected")
