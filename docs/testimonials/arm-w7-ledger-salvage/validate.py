...
def executable_record(executable: str) -> dict[str, Any]:
    invoked = shutil.which(executable)
    ...
    if executable in {"cargo", "rustc"}:
        ...
        selected = version([rustup, "which", executable])
    ...
    return {
        ...
        "version": version([invoked, *version_args]),
    }


def tool_binaries() -> dict[str, dict[str, Any]]:
    return {
        "cargo_mutants": executable_record("cargo-mutants"),
        "cargo": executable_record("cargo"),
        "rustc": executable_record("rustc"),
    }
...
def require_environment_records(records: list[dict[str, Any]]) -> None:
    require([item["name"] for item in records] == RELEVANT_ENV, "environment key set drift")
    for item in records:
...
def known_replacements(
    ...
    tools = tool_binaries() if tools is None else tools
    placeholders = {
        "cargo_mutants": "<CARGO_MUTANTS_BIN>",
        "cargo": "<CARGO_BIN>",
        "rustc": "<RUSTC_BIN>",
