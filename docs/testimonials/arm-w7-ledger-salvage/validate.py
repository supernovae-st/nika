...
def require_supported_platform() -> None:
    require(
        os.name == "posix" and sys.platform in SUPPORTED_PLATFORMS,
...
def cleanup_run_temp_root(reservation: dict[str, Any]) -> dict[str, Any]:
    ...
    require(stat.S_IMODE(metadata.st_mode) == 0o700, "temporary root mode changed")
...
def run_process(
    ...
    process = subprocess.Popen(
        argv,
        cwd=require_tool_command_cwd(pathlib.Path(RUN_WORKTREE))[0],
        env=environment,
        start_new_session=True,
    )
    returncode, monitoring = monitor_process(process, workspace_config)
    return returncode, monitoring, argv, process.pid
