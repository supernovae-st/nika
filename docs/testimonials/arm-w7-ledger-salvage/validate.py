...
TESTED_COMMIT = "200349cbde14b11cb629699a9a3fc186e3b59ae6"
TESTED_TREE = "bb7f629da770435208edd8e9c6a48cf24cad873b"
RUN_ID = "arm-w7-378fc2c6-b90d-4f5e-97ee-461fb8bdc9be"
RUN_WORKTREE = "/tmp/nika-pr1079-mutants-200349cb"
RUN_OUTPUT = f"/tmp/nika-pr1079-{RUN_ID}-output"
RUN_TARGET = f"/tmp/nika-pr1079-{RUN_ID}-target"
EXCLUDED_NAME = (
...
def execute_verified(verifier: Any, executor: Any, reprobe: Any) -> Any:
    verified = verifier()
    try:
        result = executor()
    finally:
        reprobe()
    require(result.returncode == 0, f"cargo-mutants exited {result.returncode}")
    return verified


def run_mutation(repo: pathlib.Path, receipt_commit: str) -> None:
    receipt = git(repo, "rev-parse", receipt_commit).strip()
    require(git(repo, "rev-parse", "HEAD").strip() == receipt, "atomic runner must execute from receipt HEAD")
    require(
    ...
    environment["CARGO_TARGET_DIR"] = RUN_TARGET
    execute_verified(
        lambda: verify_start(repo, receipt, require_paths_absent=True),
    ...
    print("OK: atomic preflight, cargo-mutants execution, and final clean reprobe passed")
...
def self_test() -> None:
    ...
    try:
        require_run_after_receipt("2026-08-21T04:00:00+00:00", "2026-08-21T03:59:59Z")
    except EvidenceError:
        failures += 1
    try:
        require_run_identity(RUN_ID, "/tmp/wrong-output", RUN_TARGET)
    except EvidenceError:
        failures += 1
    try:
        require_detached_head(0, "refs/heads/not-detached")
    except EvidenceError:
        failures += 1
    ...
    execute_verified(
        lambda: events.append("verify") or start,
        lambda: events.append("execute") or Result(),
        lambda: events.append("reprobe"),
    )
    require(events == ["verify", "execute", "reprobe"], "atomic wrapper order drift")
    events.clear()
    try:
        execute_verified(
            lambda: (_ for _ in ()).throw(EvidenceError("preflight red")),
            lambda: events.append("execute") or Result(),
            lambda: events.append("reprobe"),
        )
    except EvidenceError:
        failures += 1
    require(not events, "executor ran after a failed preflight")
    try:
        require_outcome_time_authority(
            {"start_time": "2026-08-21T04:00:01Z", "end_time": "2026-08-21T04:00:02Z"},
            {"start_time": "2026-08-21T04:00:00Z", "end_time": "2026-08-21T04:00:02Z"},
            "2026-08-21T03:59:59Z",
        )
    except EvidenceError:
        failures += 1
    require(failures == 8, "one of 8 negative validator mutations survived")
    print("OK: validator green fixtures passed; 8/8 negative mutations rejected")


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("command", choices=["capture-start", "verify-start", "run", "sanitize", "verify-final", "self-test"])
