...
def restore_handoff_signals(
    old_mask: set[signal.Signals] | None,
    ...
    try:
        ...
        if primary is not None:
            ...
            note_secondary(secondary, "signal-state restore failed", error)
        else:
            return error
    return secondary


def finalize_owned_process(
    process: subprocess.Popen[Any] | None,
    old_mask: set[signal.Signals] | None,
    old_handlers: dict[signal.Signals, Any],
    primary: BaseException | None,
) -> None:
    secondary = stop_owned_process(process, primary)
    secondary = restore_handoff_signals(
        old_mask, old_handlers, primary, secondary
    )
...
def monitor_process(
    ...
    try:
        ...
        try:
            ...
            primary.add_note(
                f"process-group stop failed: {type(stop_error).__name__}: {stop_error}"
            )
        raise
    monitoring = {
...
def owned_process(
    ...
    try:
        ...
        return returncode, monitoring, process.pid
    except BaseException as error:
        primary = error
        raise
    finally:
        finalize_owned_process(process, old_mask, old_handlers, primary)
