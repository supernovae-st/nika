...
def classify_startup_recovery(
    ...
    raw_exists = os.path.lexists(RAW_RUN_RECEIPT)
    if raw_exists:
        validate_resume = resume_validator or validate_raw_run_receipt
        try:
        ...
        return {"action": "resume", "roots": roots, "raw_receipt": True}
    ...
    durability = fsync_scratch_parent("startup-recovery-removal")
    absence = probe_paths_absent()
    return {
        "action": "recovered" if removed else "clean",
        "roots": roots,
        "removed": removed,
        "scratch_parent": durability,
...
def run_mutation(repo: pathlib.Path, receipt_commit: str) -> None:
    ...
    settled, _, _ = guarded_signal_temp_root(
        lambda guard: execute_mutation_transaction(
            guard, receipt, start, runtime
        )
    )
