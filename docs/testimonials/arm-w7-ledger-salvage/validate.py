...
def version(argv: list[str], environment: dict[str, str] | None = None) -> str:
    return subprocess.check_output(argv, text=True, env=environment).strip()
...
def probe_paths_absent() -> dict[str, Any]:
    ...
    require_direct_scratch_path(output, must_exist=False)
    require_direct_scratch_path(target, must_exist=False)
    require_same_bound_path(output, pathlib.Path(RUN_OUTPUT), must_exist=False)
    require_same_bound_path(target, pathlib.Path(RUN_TARGET), must_exist=False)
    result = {
...
def capture_start(repo: pathlib.Path, raw_census: pathlib.Path) -> None:
    ...
    tools = tool_binaries(environment)
    start = {
        ...
        | {"python": {"version": version(["python3", "--version"])}},
        "run": {
            ...
            "cargo_user_config": cargo_user_config_record(environment),
            "argv": RUN_ARGV,
...
def verify_start(
    ...
    require_cargo_config_record(start["run"]["cargo_user_config"])
...
def executable_record(executable: str, environment: dict[str, str]) -> dict[str, Any]:
    ...
    if executable in {"cargo", "rustc"}:
        ...
        require(rustup is not None, "rustup is required to bind the selected toolchain")
        selected = version([rustup, "which", executable], environment)
    real = pathlib.Path(selected).resolve(strict=True)
    require(stat.S_ISREG(os.lstat(real).st_mode), f"{executable} is not regular")
    version_args = ["mutants", "--version"] if executable == "cargo-mutants" else ["--version"]
    return {
        "invoked_path": invoked,
        "realpath": str(real),
        "sha256": sha256_file(real),
        "version": version([invoked, *version_args], environment),
    }


def tool_binaries(environment: dict[str, str]) -> dict[str, dict[str, Any]]:
    return {
        "cargo_mutants": executable_record("cargo-mutants", environment),
        "cargo": executable_record("cargo", environment),
        "rustc": executable_record("rustc", environment),
    }


def tool_fingerprints(tools: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return {
        name: {"sha256": record["sha256"], "version": record["version"]}
...
def require_tool_fingerprints(fingerprints: dict[str, Any]) -> None:
    require(
        set(fingerprints) == {"cargo_mutants", "cargo", "rustc", "python"},
        "start tool fingerprint set drift",
    )
    for name in ["cargo_mutants", "cargo", "rustc"]:
        require_sha256(fingerprints[name]["sha256"], f"{name} sha256 drift")
        require(bool(fingerprints[name]["version"]), f"{name} version is empty")
...
def cargo_user_config_record(environment: dict[str, str]) -> dict[str, Any]:
    ...
    require(len(present) <= 1, "ambiguous Cargo user config files")
    if not present:
        return {"source": source, "name": None, "sha256": None}
    config = present[0]
    metadata = os.lstat(config)
    require(not stat.S_ISLNK(metadata.st_mode), "Cargo user config is a symlink")
    require(stat.S_ISREG(metadata.st_mode), "Cargo user config is not regular")
    return {"source": source, "name": config.name, "sha256": sha256_file(config)}


def require_cargo_config_record(record: dict[str, Any]) -> None:
    require(record["source"] in {"CARGO_HOME", "HOME/.cargo"}, "Cargo config source drift")
    if record["name"] is None:
        require(record["sha256"] is None, "absent Cargo config has a hash")
    else:
        require(record["name"] in {"config", "config.toml"}, "Cargo config name drift")
        require_sha256(record["sha256"], "Cargo config hash drift")
...
def require_sanitized_tools(tools: dict[str, dict[str, Any]]) -> None:
    placeholders = {
        "cargo_mutants": "<CARGO_MUTANTS_BIN>",
        "cargo": "<CARGO_BIN>",
        "rustc": "<RUSTC_BIN>",
    }
    ...
    for name, placeholder in placeholders.items():
        record = tools[name]
        require(
            record["invoked_path"] == record["realpath"] == placeholder,
            f"committed {name} path is not sanitized",
        )
        require_sha256(record["sha256"], f"committed {name} sha256 drift")
        require(bool(record["version"]), f"committed {name} version is empty")
...
def require_committed_runtime_binding(
    ...
    config = run_receipt["cargo_user_config"]
    require_cargo_config_record(config)
    require(
        config == start["run"]["cargo_user_config"],
        "committed Cargo user config binding drift",
    )
...
def run_mutation(repo: pathlib.Path, receipt_commit: str) -> None:
    ...
    require(
        cargo_user_config_record(environment) == start["run"]["cargo_user_config"],
        "runner Cargo user config drifted after capture",
    )
    wall_start = now_utc()
    ...
    require_same_bound_path(
        target, pathlib.Path(RUN_TARGET), must_exist=True, directory=True
    )
    ...
    run_receipt = {
        ...
        "artifacts": {
            "outcomes.json": artifact_record(
                output,
                raw_outcomes_path,
                allowed={"outcomes.json", "mutants.out/outcomes.json"},
            ),
            "mutants.json": artifact_record(
                output,
                raw_mutants_path,
                allowed={"mutants.json", "mutants.out/mutants.json"},
            ),
        },
...
def known_replacements(tools: dict[str, dict[str, Any]]) -> dict[str, str]:
    ...
    replacements = {}
    for name, placeholder in placeholders.items():
        replacements[tools[name]["invoked_path"]] = placeholder
        replacements[tools[name]["realpath"]] = placeholder
    return replacements
...
def validate_raw_run_receipt(
    ...
    environment = build_run_environment(dict(os.environ))
    current_tools = tool_binaries(environment)
    ...
    current_config = cargo_user_config_record(environment)
    require(
        run_receipt["cargo_user_config"] == current_config,
        "raw Cargo user config binding drift",
    )
    require(
        current_config == start["run"]["cargo_user_config"],
        "raw Cargo user config drifted from start",
    )
...
def sanitize(repo: pathlib.Path, receipt_commit: str, output: pathlib.Path) -> None:
    ...
    manifest = {
        ...
        "sanitization": {
            "algorithm": "recursive replacement of only the bound cargo, cargo-mutants, and rustc executable paths, followed by Python 3 indent=2 UTF-8 serialization and one trailing newline",
