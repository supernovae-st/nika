...
def create_run_temp_root() -> dict[str, Any]:
    ...
    require(canonical == reserved, "created temporary root escaped its reservation")
...
def cleanup_run_temp_root(pre_spawn: dict[str, Any]) -> dict[str, Any]:
    root = pathlib.Path(RUN_TEMP)
    ...
    require(
        ...
        == pre_spawn["canonical_path_sha256"],
        "temporary root identity changed before cleanup",
    )
    require(
        stat.S_IMODE(os.lstat(root).st_mode) == 0o700,
        "temporary root mode changed before cleanup",
    )
    hierarchy = reject_temp_cargo_hierarchy(must_exist=True)
    entries = list(root.iterdir())
    require(not entries, "temporary root is not empty after cargo-mutants")
    root.rmdir()
    require(not os.path.lexists(root), "temporary root cleanup failed")
    return {
        "cargo_hierarchy_checked": hierarchy,
        "empty_after_process": True,
        "removed": True,
    }


def guarded_temp_root(action: Any) -> tuple[Any, dict[str, Any], dict[str, Any]]:
    pre_spawn = create_run_temp_root()
    try:
        result = action()
    finally:
        post_process = cleanup_run_temp_root(pre_spawn)
...
def run_process(
    ...
    returncode, monitoring = monitor_process(process, workspace_config)
    return returncode, monitoring, argv
...
def timed_mutation_process(
    ...
    workspace_config: dict[str, Any],
) -> tuple[str, int, int, dict[str, Any], list[str], int, str]:
    wall_start = now_utc()
    monotonic_start = time.monotonic_ns()
    returncode, monitoring, process_argv = run_process(
        environment, tools, workspace_config
    )
    return (
        wall_start,
        monotonic_start,
...
def run_mutation(repo: pathlib.Path, receipt_commit: str) -> None:
    ...
    result, temp_pre, temp_post = guarded_temp_root(
        lambda: timed_mutation_process(
            environment, tools, start["run"]["cargo_config"]["workspace"]
        )
    )
    (
        wall_start,
...
def require_temp_root_receipt(record: dict[str, Any]) -> None:
    ...
    require(post["empty_after_process"] is True, "raw temp root was not empty")
...
def require_committed_temp_root_receipt(record: dict[str, Any]) -> None:
    ...
    require(post["empty_after_process"] is True, "committed temp root was not empty")
