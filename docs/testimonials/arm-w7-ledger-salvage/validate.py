...
def launcher_record(executable: str, environment: dict[str, str]) -> dict[str, Any]:
    ...
    record |= {
        "proxy_version": version([invoked, *version_args], environment),
...
def tool_binaries(environment: dict[str, str]) -> dict[str, dict[str, Any]]:
    return {
        "cargo_mutants": launcher_record("cargo-mutants", environment),
        "cargo": launcher_record("cargo", environment),
        "rustc": launcher_record("rustc", environment),
    }
...
def require_tool_fingerprints(fingerprints: dict[str, Any]) -> None:
    ...
    for name in ["cargo_mutants", "cargo", "rustc"]:
        ...
        if name == "cargo_mutants":
            require(record["selected_sha256"] is None, "cargo-mutants selected tool drift")
            require(record["selected_version"] is None, "cargo-mutants selected version drift")
        else:
...
def require_sanitized_tools(tools: dict[str, dict[str, Any]]) -> None:
    ...
    for name, (invoked, proxy, selected_path) in placeholders.items():
        ...
        selected = record["selected_toolchain"]
        if name == "cargo_mutants":
            require(selected is None, "cargo-mutants has a selected toolchain")
        else:
...
def run_process(
    environment: dict[str, str], expected_tools: dict[str, dict[str, Any]]
    ...
    process = subprocess.Popen(argv, cwd=RUN_WORKTREE, env=environment)
    probe_count = 0
    violation = None
    try:
        while True:
    ...
    return int(returncode), monitoring, argv
...
def run_mutation(repo: pathlib.Path, receipt_commit: str) -> None:
    ...
    returncode, monitoring, process_argv = run_process(environment, tools)
