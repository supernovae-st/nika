...
| 38 | `check-public-api-coverage.sh` | Public Rust API coverage ratchet. Workspace library targets define the expected set; binary-only packages have no surface. The monotonic floor in `scripts/ci/public-api-coverage-baseline.txt` is shared by hygiene, public API diff and semver checks. A missing covered snapshot is RED; an uncovered library is YELLOW. Generate snapshots on Linux with the pinned rustdoc and cargo-public-api versions and default features. |
