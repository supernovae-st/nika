...
printf '%s\n' "$finalizer" | grep -q 'resolve-release-tag.sh' \
  || fail 'finalizer does not re-resolve the tag'
[ "$(grep -c 'gh release edit .*--repo' "$ROOT/.github/workflows/release.yml")" -eq 1 ] \
  || fail 'there is not exactly one GitHub release visibility publisher'
[ "$(grep -c -- '--draft=false' "$ROOT/.github/workflows/release.yml")" -eq 1 ] \
  || fail 'there is not exactly one draft finalization call'
...
printf '%s\n' "$latest_job" | grep -q "github.event_name == 'push'" \
  || fail 'manual replay can move GHCR latest'
