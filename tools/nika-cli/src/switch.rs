...
// ═══════════════════════════════════════════════════════════════════════════
// SOLARIZED PALETTE (colored crate CustomColor)
// ═══════════════════════════════════════════════════════════════════════════
...
fn detect_release() -> Option<ChannelInfo> {
    // Check Homebrew path first, then fallback to which
    let candidates = [
...
fn do_switch(nika_dir: &Path, target: &str, quiet: bool) -> Result<(), NikaError> {
    ...
    let target_binary = match target {
        "dev" => {
            ...
            if !p.exists() {
                return Err(NikaError::ValidationError {
                    reason: "Dev binary not found. Run `nika switch --build` first.".into(),
                });
            }
            p
        }
...
fn print_channel_line(name: &str, info: &Option<ChannelInfo>, active: &str) {
    ...
    let (name_color, dot_str, label_str) = if is_active {
...
fn animate_transition(from: &str, to: &str, _nika_dir: &Path) -> Result<(), NikaError> {
    let release = detect_release();
    let dev = detect_dev();

    // Frame 1: departing state
    println!();
    print_compact_line("release", &release, from);
    print_compact_line("dev", &dev, from);
    std::thread::sleep(Duration::from_millis(150));
    clear_lines(3);

    // Frame 2: spark traveling
    println!();
...
mod tests {
    ...
    fn test_install_hook_idempotent() {
        ...
        let count = hook.matches(HOOK_MARKER).count();
        // The script has the marker once in its content, plus the one-line comment = 2
        // Actually HOOK_SCRIPT contains HOOK_MARKER once. The written file has #!/bin/sh + HOOK_SCRIPT
        assert_eq!(count, 1);
    }
