...
const SOL_CYAN: CustomColor = CustomColor { r: 42, g: 161, b: 152 };
const SOL_MAGENTA: CustomColor = CustomColor { r: 211, g: 54, b: 130 };
const SOL_GREEN: CustomColor = CustomColor { r: 133, g: 153, b: 0 };
const SOL_DIM: CustomColor = CustomColor { r: 88, g: 110, b: 117 };
const SOL_YELLOW: CustomColor = CustomColor { r: 181, g: 137, b: 0 };
const SOL_RED: CustomColor = CustomColor { r: 220, g: 50, b: 47 };
  ...
  let target_binary = match target {
    ...
    "release" => {
      let info = detect_release().ok_or_else(|| NikaError::ValidationError {
        reason: "Release binary not found. Install with: brew install supernovae-st/tap/nika"
          .into(),
      })?;
      info.binary_path
    }
...
fn print_channel_line(name: &str, info: &Option<ChannelInfo>, active: &str) {
  ...
  let name_color = if is_active {
    if name == "release" { SOL_CYAN } else { SOL_MAGENTA }
  } else {
    SOL_DIM
  };
...
fn print_compact_line(name: &str, info: &Option<ChannelInfo>, active: &str) {
  ...
  let dot = if is_active { "●" } else { "○" };
  let label = if is_active { "━━━━ active" } else { "" };
  let nc = if name == "release" { SOL_CYAN } else { SOL_MAGENTA };
...
mod tests {
  ...
  fn test_build_meta_parse() {
    let json =
      r#"{"version":"0.51.0","hash":"abc1234","built_at":"2026-03-30T14:23:00Z","status":"ok"}"#;
