...
mod tests {
  ...
  fn extract_mode_parse_all() {
    for name in ExtractMode::ALL_NAMES {
      assert!(ExtractMode::parse(name).is_some(), "failed to parse '{name}'");
    }
  ...
  fn response_mode_parse_all() {
    for name in ResponseMode::ALL_NAMES {
      assert!(ResponseMode::parse(name).is_some(), "failed to parse '{name}'");
    }
  ...
  fn extract_mode_required_features() {
    assert_eq!(ExtractMode::Markdown.required_feature(), Some("fetch-markdown"));
    assert_eq!(ExtractMode::Article.required_feature(), Some("fetch-article"));
