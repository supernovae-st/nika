...
//!
//! Moved from nika-kernel-mock in S24 to break the cargo dev-dep triangle
//! (nika-kernel-mock → nika-core → nika-kernel-mock) that previously blocked
//! migrating MockBindingStore-based tests into nika-core.
...
use crate::binding::{BindingStore, BindingStoreError};
...
mod tests {
    ...
    fn with_output_roundtrip() {
        ...
        let output = store.get_output("weather").expect("output should exist");
