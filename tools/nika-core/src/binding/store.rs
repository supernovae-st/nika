...
//! enabling the binding resolver to live in nika-core instead of the engine
//! monolith. RunContext implements this trait in nika-engine; tests use the
//! `MockBindingStore` from `nika-kernel-mock`.
//!
//! # Location (S23)
//!
//! This trait originally lived in `nika-kernel::binding`. It was moved here
//! (nika-core L0) so that the binding resolver (`binding/resolve.rs`) could
//! follow without introducing a circular dependency (nika-kernel already
//! depends on nika-core). `nika-kernel::binding` still re-exports the trait
//! and error for backwards compatibility with downstream code.
//!
//! # Methods (9)
//!
