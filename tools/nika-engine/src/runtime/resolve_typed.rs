...
pub fn resolve_task_bool(
  ...
  match field {
    ...
    Some(Templatable::Template(tpl)) => {
      let resolved = resolve_template_str(tpl, bindings, ctx)?;
      let val = parse_bool_value(resolved.trim()).ok_or_else(|| NikaError::BindingTypeMismatch {
        path: name.to_string(),
...
pub fn resolve_required_u64(
  ...
  match field {
    ...
    Templatable::Template(tpl) => {
      ...
      trimmed.parse::<u64>().map_err(|_| NikaError::BindingTypeMismatch {
        path: name.to_string(),
...
mod tests {
  ...
  fn test_resolve_task_u32_value() {
    ...
    assert_eq!(resolve_task_u32(&field, &bindings, &ctx, "x").unwrap(), Some(5));
  }
  ...
  fn test_resolve_task_bool_value() {
    ...
    assert_eq!(resolve_task_bool(&field, &bindings, &ctx, "x").unwrap(), Some(true));
  }
