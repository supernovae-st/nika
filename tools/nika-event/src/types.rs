...
mod tests {
  ...
  fn finish_reason_known_values() {
    ...
    assert_eq!(FinishReason::from("structured_output_retry"), FinishReason::StructuredOutputRetry);
  }

  #[test]
  fn finish_reason_unknown_passthrough() {
    let reason = FinishReason::from("limit_exceeded:cost");
    assert_eq!(reason, FinishReason::Other("limit_exceeded:cost".to_string()));
  ...
  fn agent_stop_reason_known_values() {
    ...
    assert_eq!(AgentStopReason::from("max_turns"), AgentStopReason::MaxTurns);
    assert_eq!(AgentStopReason::from("guardrail_retry"), AgentStopReason::GuardrailRetry);
  }

  #[test]
  fn agent_stop_reason_unknown_passthrough() {
    let reason = AgentStopReason::from("Completed { success: true }");
    assert_eq!(reason, AgentStopReason::Other("Completed { success: true }".to_string()));
  }
}
