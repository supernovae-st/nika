...
//!
//! Nine extract modes: `markdown`, `article`, `text`, `selector`, `metadata`,
//! `links`, `metadata_links`, `jsonpath`, `feed`, `llm_txt`, `sitemap`.
//!
//! Moved verbatim from `nika-engine/runtime/executor/extract.rs` in Session 12
//! of the Constellation refactor. This crate is pure: no I/O, no async, no
//! locks — only byte→structured-output transformation.

use nika_core::ast::extract::ExtractMode;

/// Extraction errors, independent of `NikaError` so this crate stays L2.
#[derive(Debug, thiserror::Error)]
pub enum ExtractError {
    #[error("extract error: {0}")]
    Failed(String),
    #[error("extract: mode '{0}' requires feature flag not enabled at build time")]
...
pub fn parse_link_header_hreflang(link_values: &[String]) -> Vec<serde_json::Value> {
...
// Feature-disabled fallback error helper (only used when a fetch-* feature
// is disabled at build time).
#[allow(dead_code)]
fn feature_disabled(mode: &'static str) -> ExtractError {
    ExtractError::FeatureDisabled(mode)
}

/// Apply an extraction mode to an HTTP response body.
///
/// - `body`: the response body as UTF-8 text
...
///
/// This is the canonical public entry point. [`apply_extract_with_base`] is
/// kept for internal call compatibility during the Constellation refactor.
pub fn extract(
    body: &str,
    ...
    apply_extract_with_base(body, extract, selector, base_url)
}
    ...
    match mode {
        ...
        ExtractMode::Selector => {
            let css = selector.ok_or_else(|| {
                ExtractError::Failed("extract: selector requires 'selector' field".to_string())
            })?;
        ...
        ExtractMode::MetadataLinks => {
            ...
            let mut meta: serde_json::Value = serde_json::from_str(&meta_str)
                .map_err(|e| ExtractError::Failed(format!("JSON parse: {e}")))?;
            ...
            serde_json::to_string(&meta)
                .map_err(|e| ExtractError::Failed(format!("JSON serialize: {e}")))
        }
        ...
        ExtractMode::Article => {
            let mut readability =
                dom_smoothie::Readability::new(body, None, None)
                    .map_err(|e| ExtractError::Failed(format!("Readability init failed: {e}")))?;
            let article = readability.parse().map_err(|e| {
                ExtractError::Failed(format!("Readability parse failed: {e}"))
            })?;
            Ok(serde_json::json!({
                "title": article.title,
        ...
        ExtractMode::Feed => {
            let feed = feed_rs::parser::parse(body.as_bytes())
                .map_err(|e| ExtractError::Failed(format!("Feed parse failed: {e}")))?;
        ...
        ExtractMode::Sitemap => Err(feature_disabled("sitemap")),

        #[cfg(not(feature = "fetch-markdown"))]
        ExtractMode::Markdown => Err(feature_disabled("markdown")),
        #[cfg(not(feature = "fetch-html"))]
        ExtractMode::Text | ExtractMode::Selector | ExtractMode::Metadata | ExtractMode::Links | ExtractMode::MetadataLinks => {
            Err(feature_disabled("text/selector/metadata/links/metadata_links"))
        }
        #[cfg(not(feature = "fetch-article"))]
        ExtractMode::Article => Err(feature_disabled("article")),
        #[cfg(not(feature = "fetch-feed"))]
        ExtractMode::Feed => Err(feature_disabled("feed")),
    }
}
...
#[cfg(feature = "fetch-html")]
fn extract_text(html: &str, selector: Option<&str>) -> Result<String, ExtractError> {
    ...
    if let Some(css) = selector {
        let sel = scraper::Selector::parse(css)
            .map_err(|_| ExtractError::Failed(format!("Invalid CSS selector: {css}")))?;
...
#[cfg(feature = "fetch-html")]
fn extract_html_by_selector(html: &str, css: &str) -> Result<String, ExtractError> {
...
#[cfg(feature = "fetch-html")]
fn extract_metadata_json(html: &str, base_url: Option<&str>) -> Result<String, ExtractError> {
    let document = scraper::Html::parse_document(html);
    ...
    serde_json::to_string(&meta).map_err(|e| ExtractError::Failed(format!("JSON serialize: {e}")))
}

#[cfg(feature = "fetch-html")]
fn extract_links_json(html: &str, base_url: Option<&str>) -> Result<String, ExtractError> {
    let document = scraper::Html::parse_document(html);
...
fn extract_jsonpath(body: &str, path: &str) -> Result<String, ExtractError> {
    let json: serde_json::Value =
        serde_json::from_str(body).map_err(|e| {
            ExtractError::Failed(format!("Response is not valid JSON: {e}"))
        })?;
    let jsonpath = serde_json_path::JsonPath::parse(path)
        .map_err(|e| ExtractError::Failed(format!("Invalid JSONPath '{}': {e}", path)))?;
    let results: Vec<&serde_json::Value> = jsonpath.query(&json).all();
    match results.len() {
        0 => Ok("null".to_string()),
        1 => serde_json::to_string(results[0])
...
#[cfg(feature = "fetch-sitemap")]
fn extract_sitemap_xml(body: &str) -> Result<String, ExtractError> {
    ...
    serde_json::to_string(&result).map_err(|e| ExtractError::Failed(format!("JSON serialize: {e}")))
}
...
mod tests {
    ...
    fn error_display_contains_reason() {
        ...
        let e2 = ExtractError::FeatureDisabled("article");
        assert!(e2.to_string().contains("article"));
    }

    #[test]
    fn extract_entry_point_delegates_to_with_base() {
        // Public `extract()` must behave identically to `apply_extract_with_base`.
        let result_direct =
            apply_extract_with_base("<html></html>", None, None, None).unwrap();
        let result_entry = extract("<html></html>", None, None, None).unwrap();
        assert_eq!(result_direct, result_entry);
    }
