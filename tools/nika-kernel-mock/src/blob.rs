...
mod tests {
    ...
    async fn put_and_get_roundtrip() {
        ...
        let meta = blob.put(Bytes::from_static(b"hello"), "text/plain").await.unwrap();
    ...
    async fn put_counter_produces_different_hashes() {
        let blob = MockBlob::new();
        let m1 = blob.put(Bytes::from_static(b"aaa"), "text/plain").await.unwrap();
        let m2 = blob.put(Bytes::from_static(b"bbb"), "text/plain").await.unwrap();
