...
use nika_kernel::tool_executor::{
    ToolBatch, ToolCall, ToolExecError, ToolExecute, ToolResult,
};
...
mod tests {
    ...
    async fn mock_enqueue_and_execute() {
        let executor = MockToolExecutor::new()
            .enqueue_ok(ToolResult::success("tc1", "file contents"));
        let call = ToolCall::new("tc1", "nika:read", serde_json::json!({}));
    ...
    async fn mock_records_calls() {
        let executor = MockToolExecutor::new()
            .enqueue_ok(ToolResult::success("tc1", "ok"));
        let _ = executor
            .execute(ToolCall::new("tc1", "nika:read", serde_json::json!({"path": "/tmp"})))
            .await;
    ...
    async fn mock_enqueue_err_populates_queue() {
        let executor = MockToolExecutor::new()
            .enqueue_err(ToolExecError::NotFound { name: "nika:ghost".into() });
        let result = executor
