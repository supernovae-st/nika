...
mod tests {
    ...
    fn provider_error_codes_in_range() {
        ...
        let code = err.nika_code();
        assert!(code.num >= 380 && code.num <= 429, "provider code {}", code.num);
    ...
    fn memory_error_codes_in_range() {
        ...
        let code = err.nika_code();
        assert!(code.num >= 600 && code.num <= 649, "memory code {}", code.num);
    }
    ...
    fn all_provider_variants_have_codes() {
        let _ = ProviderError::Api { status: 500, message: "".into() }.nika_code();
        let _ = ProviderError::ModelNotFound { model: "".into() }.nika_code();
        let _ = ProviderError::RateLimited { retry_after_ms: None }.nika_code();
    ...
    fn all_memory_variants_have_codes() {
        ...
        let _ = MemoryError::NotFound { id: MemoryId::nil() }.nika_code();
