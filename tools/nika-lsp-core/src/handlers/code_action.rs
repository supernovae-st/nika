...
pub fn code_actions(text: &str, start_offset: u32, _end_offset: u32) -> Vec<CodeActionEntry> {
    ...
    if !text.contains("schema:") {
        actions.push(CodeActionEntry {
            title: "Add schema version".into(),
            kind: CodeActionKind::QuickFix,
            is_preferred: true,
            edit: Some(TextEdit {
                offset: 0,
                end_offset: 0,
                new_text: "schema: nika/workflow@0.12\n".into(),
            }),
        });
    }
    ...
    if let Some(rest) = trimmed.strip_prefix("infer:") {
        ...
        if !prompt.is_empty() && !prompt.contains('\n') {
            ...
            actions.push(CodeActionEntry {
                title: "Expand shorthand infer".into(),
...
mod tests {
    ...
    #[test]
    fn missing_schema() {
        assert!(code_actions("workflow: x\n", 0, 0)
            .iter()
            .any(|a| a.title.contains("schema")));
    }
    #[test]
    fn has_schema() {
        assert!(!code_actions("schema: x\n", 0, 0)
            .iter()
            .any(|a| a.title.contains("schema")));
    }
    #[test]
    fn expand_infer() {
        let t = "    infer: \"hi\"\n";
        let o = t.find("infer").unwrap() as u32;
        assert!(code_actions(t, o, o)
            .iter()
            .any(|a| a.title.contains("Expand")));
    }
}
