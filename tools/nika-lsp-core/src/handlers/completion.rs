...
fn verb_block_completions(verb: &str, prefix: &str) -> Vec<CompletionItem> {
    let items = match verb {
        ...
        "fetch" => vec![
            ...
            item_snippet_fmt("extract", CompletionItemKind::PROPERTY, "extract: ${1|markdown,article,text,selector,metadata,links,jsonpath,feed,llm_txt|}", "Post-processing extraction mode (9 modes).", "4_extract"),
