...
pub fn hover(_text: &str, _offset: u32, context: &CursorContext) -> Option<HoverResult> {
  match context {
    ...
    CursorContext::WithBlock { .. } => Some(HoverResult { contents: "## `with:` — Data Bindings\n\nBind outputs from upstream tasks.".to_string(), range: None }),
    CursorContext::Template { in_transform_chain: true, partial_expr, .. } => transform_hover(partial_expr),
    CursorContext::Template { .. } => Some(HoverResult { contents: "## Template Expression\n\nAccess data: `{{with.alias}}`, `{{inputs.param}}`".to_string(), range: None }),
    _ => None,
  }
}
...
fn verb_hover(verb: &str) -> Option<HoverResult> {
  ...
  Some(HoverResult { contents: doc.to_string(), range: None })
}
...
fn transform_hover(expr: &str) -> Option<HoverResult> {
  ...
  let doc = match t { "upper" => "UPPERCASE", "lower" => "lowercase", "trim" => "Strip whitespace", _ => return None };
  Some(HoverResult { contents: format!("`{t}` — {doc}"), range: None })
}
...
mod tests {
  use super::*;
  #[test]
  fn all_verbs() { for v in ["infer","exec","fetch","invoke","agent"] { assert!(verb_hover(v).is_some()); } }
  #[test]
  fn unknown_none() { let c = CursorContext::Unknown{prefix:String::new()}; assert!(hover("",0,&c).is_none()); }
  #[test]
  fn content_hover_type() { use crate::analysis::context::ContentFocus; assert!(content_hover(&ContentFocus::PartType).is_some()); }
}
