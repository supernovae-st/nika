...
//! - Network access (`fetch:` verb, including SSRF + DNS rebinding)
//! - Token budget limits
//! - Host restrictions (allowed_hosts, blocked_hosts)
//!
//! The concrete [`PolicyEnforcer`] also implements the object-safe
//! [`nika_kernel::policy::PolicyChecker`] trait, so verb crates can depend
//! on the trait and remain fully decoupled from this crate.
...
use nika_core::policy::PolicyConfig;
use url::Url;
...
pub async fn resolve_and_pin_ssrf(host: &str) -> Result<Vec<std::net::SocketAddr>, String> {
...
impl PolicyEnforcer {
    ...
    /// Convert a [`PolicyDecision`] to a [`Result`], independent of `NikaError`.
    ///
    /// Engine-side callers that need to propagate `NikaError` can chain
    /// `?` with the `From<nika_policy::PolicyError> for NikaError` impl.
    pub fn enforce(&self, decision: PolicyDecision) -> Result<(), PolicyError> {
        match decision {
            PolicyDecision::Allow => Ok(()),
            PolicyDecision::Block(reason) => Err(PolicyError::Violation(reason)),
            PolicyDecision::RequiresApproval(reason) => {
                Err(PolicyError::Violation(format!("Requires approval: {}", reason)))
            }
        }
    }
}
...
/// Map a crate-local [`PolicyDecision`] to the kernel-level one.
fn map_decision(d: PolicyDecision) -> nika_kernel::policy::PolicyDecision {
    match d {
...
mod tests {
    ...
    fn test_custom_endpoint_ip_auto_allowed_in_policy() {
        // Simulate extracting hosts from custom endpoints (mirrors engine's
        // `with_policy` helper without pulling in nika-engine::provider types).
        let mut policy = PolicyConfig::default();
        let endpoint_base_urls = ["http://10.0.1.42:8000/v1", "http://192.168.1.100:11434/v1"];
        for base_url in endpoint_base_urls {
    ...
    fn policy_checker_trait_impl_routes_through_concrete() {
        ...
        let d: KD = PolicyChecker::check_exec(&enforcer, "any cmd");
        assert!(matches!(d, KD::Block(_)));

        // is_host_allowed: empty allowed_hosts → false for arbitrary host
        assert!(!PolicyChecker::is_host_allowed(&enforcer, "example.com"));

        let enforcer2 = PolicyEnforcer::new(PolicyConfig {
            allowed_hosts: vec!["example.com".into()],
            ..PolicyConfig::default()
        });
        assert!(PolicyChecker::is_host_allowed(&enforcer2, "EXAMPLE.COM"));
    }
