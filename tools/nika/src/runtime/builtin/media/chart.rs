...
impl MediaOp for ChartOp {
  ...
  fn execute<'a>(
    ...
    Box::pin(async move {
      ...
      let chart_type = args.get("type").and_then(|v| v.as_str())
        .ok_or_else(|| invalid_args("chart", "missing 'type' (bar, line, or pie)"))?;

      let series_arr = args.get("series").and_then(|v| v.as_array())
      ...
      let width = args.get("width").and_then(|v| v.as_u64()).unwrap_or(800).clamp(100, 4000) as f32;
      let height = args.get("height").and_then(|v| v.as_u64()).unwrap_or(500).clamp(100, 4000) as f32;
      ...
      let png_data = ctx.compute.compute(move || -> Result<Vec<u8>, NikaError> {
        let svg = render_chart_svg(&chart_type_owned, &title, width, height, series_list, &labels)?;
...
fn parse_series(arr: &[serde_json::Value]) -> Result<Vec<charts_rs::Series>, NikaError> {
  ...
  for (i, item) in arr.iter().enumerate() {
    ...
    let data = item.get("data").and_then(|v| v.as_array())
    ...
    if values.is_empty() {
      return Err(invalid_args("chart", format!("series[{i}].data must not be empty")));
    }
...
fn render_chart_svg(
  ...
  match chart_type {
    ...
    other => Err(invalid_args("chart", format!("unsupported chart type '{other}', use bar, line, or pie"))),
  }
}
...
mod tests {
  ...
  async fn chart_dimension_clamped() {
    ...
    if let MediaOpResult::Binary { metadata, .. } = result {
      ...
      assert_eq!(metadata["height"], 4000, "height should be clamped to max 4000");
    }
  }
}
