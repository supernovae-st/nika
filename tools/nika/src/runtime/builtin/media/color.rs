  ...
  fn execute<'a>(
    ...
    Box::pin(async move {
      let hash = args
        .get("hash")
        .and_then(|v| v.as_str())
        .ok_or_else(|| invalid_args("dominant_color", "missing required parameter 'hash'"))?;
      ...
      let colors = ctx.compute.compute(move || {
        extract_palette(&data, count, quality)
      }).await??;
...
mod tests {
  ...
  async fn dominant_color_solid_red() {
    ...
    if let MediaOpResult::Metadata(v) = result {
      ...
      let first = &colors[0];
      assert!(first["r"].as_u64().unwrap() > 200, "red channel should be high");
    } else {
