...
impl MediaOp for MetadataOp {
  ...
  fn execute<'a>(
    ...
    Box::pin(async move {
      ...
      let metadata = ctx.compute.compute(move || {
        extract_metadata(&data, &mime, size)
      }).await?;
...
fn extract_exif(data: &[u8], map: &mut serde_json::Map<String, serde_json::Value>) {
  ...
  for entry in exif_iter {
    ...
    let value_str = entry.get_value().map(|v| format!("{v:?}")).unwrap_or_default();
...
mod tests {
  ...
  fn fixture_png() -> Vec<u8> {
    ...
    image::ImageEncoder::write_image(
      encoder, img.as_raw(), 10, 20, image::ExtendedColorType::Rgb8,
    ).unwrap();
    buf
  }
