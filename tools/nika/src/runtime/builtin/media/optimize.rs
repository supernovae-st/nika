...
use super::error::{invalid_args, unsupported_format, tool_error};
...
impl MediaOp for OptimizeOp {
  ...
  fn execute<'a>(
    ...
    Box::pin(async move {
      ...
      let optimized = ctx.compute.compute(move || -> Result<Vec<u8>, NikaError> {
...
mod tests {
  ...
  fn fixture_png() -> Vec<u8> {
    ...
    image::ImageEncoder::write_image(
      encoder, img.as_raw(), 50, 50, image::ExtendedColorType::Rgb8,
    ).unwrap();
    buf
  }
