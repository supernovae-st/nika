...
mod tests {
  ...
  async fn t01_thumbnail_solid_blue_pixels_are_blue() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 40}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap().to_rgba8();
      for pixel in img.pixels() {
        assert!(pixel[2] > 200, "blue channel should be > 200, got {}", pixel[2]);
        assert!(pixel[0] < 50, "red channel should be < 50, got {}", pixel[0]);
        assert!(pixel[1] < 50, "green channel should be < 50, got {}", pixel[1]);
      }
    } else {
  ...
  async fn t02_convert_roundtrip_pixel_fidelity() {
    ...
    let op = ConvertOp;
    // PNG -> JPEG
    let jpeg_result = op.execute(serde_json::json!({"hash": hash, "format": "jpeg", "quality": 100}), &ctx).await.unwrap();
    let jpeg_hash = if let MediaOpResult::Binary { data, .. } = &jpeg_result {
      store(&ctx, data).await
    } else { panic!("expected Binary") };
    // JPEG -> PNG
    let png_result = op.execute(serde_json::json!({"hash": jpeg_hash, "format": "png"}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = png_result {
      ...
      assert!(center[0] > 240, "red channel after roundtrip should be > 240, got {}", center[0]);
      assert!(center[1] < 30, "green channel after roundtrip should be < 30, got {}", center[1]);
      assert!(center[2] < 30, "blue channel after roundtrip should be < 30, got {}", center[2]);
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t03_strip_pixel_perfect_lossless() {
    ...
    let op = StripOp;
    let result = op.execute(serde_json::json!({"hash": hash}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
  ...
  async fn t04_thumbnail_checkerboard_2to1_pattern_integrity() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 50}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap().to_rgba8();
      ...
      assert!(tl[0] < 50 && tl[1] < 50 && tl[2] < 50,
        "top-left should be dark, got {:?}", tl);
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t05_convert_transparent_png_to_jpeg_white_composite() {
    ...
    let op = ConvertOp;
    let result = op.execute(serde_json::json!({"hash": hash, "format": "jpeg"}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
  ...
  async fn t06_thumbnail_preserves_alpha_values() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 20, "format": "png"}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
  ...
  async fn t07_thumbnail_jpeg_input_pixel_check() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 30}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap().to_rgba8();
      assert_eq!(img.width(), 30);
      assert_eq!(img.height(), 15);
      let center = img.get_pixel(15, 7);
      assert!(center[1] > 150, "green channel should be dominant, got {}", center[1]);
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t08_optimize_pixel_equivalence_lossless() {
    ...
    if let MediaOpResult::Binary { data, .. } = result {
      ...
      for (o, s) in orig_img.pixels().zip(opt_img.pixels()) {
        assert_eq!(o, s, "optimize should be lossless: original {:?} != optimized {:?}", o, s);
      }
    } else {
      panic!("expected Binary");
    }
  ...
  async fn t10_svg_render_pixel_at_fill_region() {
    ...
    let result = op.execute(serde_json::json!({"hash": hash, "width": 100, "height": 100}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap().to_rgba8();
      let center = img.get_pixel(50, 50);
      assert!(center[0] > 240, "red fill should be visible at center, R={}", center[0]);
      assert!(center[3] > 240, "alpha should be opaque at center, A={}", center[3]);
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t11_thumbnail_odd_ratio_7x3_to_5() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 5}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, metadata, .. } = result {
      let img = image::load_from_memory(&data).unwrap();
      assert_eq!(img.width(), 5);
      let h = img.height();
      assert!(h == 2 || h == 3, "7:3 ratio at width=5 -> height ~2, got {h}");
  ...
  async fn t12_thumbnail_prime_dimensions_13x11_to_7() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 7}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap();
      assert_eq!(img.width(), 7);
      // 7 * 11/13 = 5.923 -> ~6
      let h = img.height();
      assert!((5..=7).contains(&h), "expected height ~6 for 13:11 at w=7, got {h}");
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t13_thumbnail_extreme_landscape_to_1px() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 1}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, metadata, .. } = result {
      let img = image::load_from_memory(&data).unwrap();
  ...
  async fn t14_thumbnail_extreme_portrait_height_at_limit() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 10}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, metadata, .. } = result {
      let img = image::load_from_memory(&data).unwrap();
      assert_eq!(img.width(), 10);
      let h = metadata["height"].as_u64().unwrap();
      assert!(h <= 10_000, "height must not exceed 10000, got {h}");
      assert!(h >= 1000, "height should be ~10000 for 1:1000 at w=10, got {h}");
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t17_thumbnail_3x3_to_2() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 2}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap();
  ...
  async fn t18_thumbnail_non_round_ratio_500x250_to_333() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 333}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap();
      assert_eq!(img.width(), 333);
      let h = img.height();
      assert!((166..=167).contains(&h), "500:250 at w=333 -> h~166.5, got {h}");
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t19_thumbnail_upscale_2x2_to_100() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 100}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      let img = image::load_from_memory(&data).unwrap();
  ...
  async fn t20_thumbnail_large_downscale_5000_to_10() {
    ...
    if let MediaOpResult::Binary { data, .. } = result {
      ...
      let p = img.to_rgba8().get_pixel(5, 2).clone();
  ...
  async fn t21_convert_webp_magic_bytes_structure() {
    ...
    let op = ConvertOp;
    let result = op.execute(serde_json::json!({"hash": hash, "format": "webp"}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
  ...
  async fn t22_convert_jpeg_soi_eoi_markers() {
    ...
    let result = op.execute(serde_json::json!({"hash": hash, "format": "jpeg"}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      assert_eq!(&data[..2], &[0xFF, 0xD8], "JPEG should start with SOI marker");
      assert_eq!(&data[data.len()-2..], &[0xFF, 0xD9], "JPEG should end with EOI marker");
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t23_thumbnail_png_signature_and_ihdr() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 20}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = result {
      // PNG signature: 8 bytes
      assert_eq!(&data[..8], &[137, 80, 78, 71, 13, 10, 26, 10], "PNG signature");
  ...
  async fn t26_convert_webp_to_png_chain() {
    ...
    let op = ConvertOp;
    // PNG -> WebP
    let webp = op.execute(serde_json::json!({"hash": hash, "format": "webp"}), &ctx).await.unwrap();
    let webp_hash = if let MediaOpResult::Binary { data, .. } = &webp {
      store(&ctx, data).await
    } else { panic!() };
    // WebP -> PNG
    let png = op.execute(serde_json::json!({"hash": webp_hash, "format": "png"}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, mime_type, .. } = png {
  ...
  async fn t27_convert_three_format_chain_dimensions_stable() {
    ...
    let op = ConvertOp;
    // PNG -> JPEG
    let r1 = op.execute(serde_json::json!({"hash": hash, "format": "jpeg"}), &ctx).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };
    // JPEG -> WebP
    let r2 = op.execute(serde_json::json!({"hash": h1, "format": "webp"}), &ctx).await.unwrap();
    let h2 = if let MediaOpResult::Binary { data, .. } = &r2 { store(&ctx, data).await } else { panic!() };
    // WebP -> PNG
    let r3 = op.execute(serde_json::json!({"hash": h2, "format": "png"}), &ctx).await.unwrap();
  ...
  async fn t29_strip_format_override_jpeg_to_png() {
    ...
    let op = StripOp;
    let result = op.execute(
      serde_json::json!({"hash": hash, "format": "png"}),
      &ctx,
    ).await.unwrap();
    if let MediaOpResult::Binary { data, mime_type, .. } = result {
      assert_eq!(mime_type, "image/png");
  ...
  async fn t30_thumbnail_jpeg_vs_png_different_sizes() {
    ...
    let op = ThumbnailOp;
    let png_result = op.execute(serde_json::json!({"hash": hash, "width": 100, "format": "png"}), &ctx).await.unwrap();
    let jpeg_result = op.execute(serde_json::json!({"hash": hash, "width": 100, "format": "jpeg"}), &ctx).await.unwrap();
    let png_size = if let MediaOpResult::Binary { data, .. } = png_result { data.len() } else { 0 };
    let jpeg_size = if let MediaOpResult::Binary { data, .. } = jpeg_result { data.len() } else { 0 };
    assert_ne!(
      jpeg_size, png_size,
  ...
  async fn t31_thumbnail_idempotent_binary_output() {
    ...
    let op = ThumbnailOp;
    let r1 = op.execute(serde_json::json!({"hash": hash, "width": 40}), &ctx).await.unwrap();
    let r2 = op.execute(serde_json::json!({"hash": hash, "width": 40}), &ctx).await.unwrap();
    let d1 = if let MediaOpResult::Binary { data, .. } = r1 { data } else { panic!() };
    let d2 = if let MediaOpResult::Binary { data, .. } = r2 { data } else { panic!() };
    assert_eq!(d1, d2, "identical thumbnail operations must produce identical binary output");
  }
  ...
  async fn t32_convert_idempotent_binary_output() {
    ...
    let op = ConvertOp;
    let r1 = op.execute(serde_json::json!({"hash": hash, "format": "jpeg", "quality": 80}), &ctx).await.unwrap();
    let r2 = op.execute(serde_json::json!({"hash": hash, "format": "jpeg", "quality": 80}), &ctx).await.unwrap();
    let d1 = if let MediaOpResult::Binary { data, .. } = r1 { data } else { panic!() };
    let d2 = if let MediaOpResult::Binary { data, .. } = r2 { data } else { panic!() };
    assert_eq!(d1, d2, "identical convert operations must produce identical binary output");
  }
  ...
  async fn t34_strip_idempotent() {
    ...
    let op = StripOp;
    let r1 = op.execute(serde_json::json!({"hash": hash}), &ctx).await.unwrap();
    let d1 = if let MediaOpResult::Binary { data, .. } = r1 { data } else { panic!() };
    let h1 = store(&ctx, &d1).await;
    // Strip the already-stripped output
    let r2 = op.execute(serde_json::json!({"hash": h1}), &ctx).await.unwrap();
    let d2 = if let MediaOpResult::Binary { data, .. } = r2 { data } else { panic!() };
  ...
  async fn t35_thumbnail_cas_hash_deterministic() {
    ...
    let op = ThumbnailOp;
    let r1 = op.execute(serde_json::json!({"hash": hash, "width": 25}), &ctx).await.unwrap();
    let r2 = op.execute(serde_json::json!({"hash": hash, "width": 25}), &ctx).await.unwrap();
    let d1 = if let MediaOpResult::Binary { data, .. } = &r1 { data.clone() } else { panic!() };
    let d2 = if let MediaOpResult::Binary { data, .. } = &r2 { data.clone() } else { panic!() };
    let h1 = store(&ctx, &d1).await;
  ...
  async fn t36_thumbnail_different_widths_different_hashes() {
    ...
    let op = ThumbnailOp;
    let r1 = op.execute(serde_json::json!({"hash": hash, "width": 30}), &ctx).await.unwrap();
    let r2 = op.execute(serde_json::json!({"hash": hash, "width": 50}), &ctx).await.unwrap();
    let d1 = if let MediaOpResult::Binary { data, .. } = r1 { data } else { panic!() };
    let d2 = if let MediaOpResult::Binary { data, .. } = r2 { data } else { panic!() };
  ...
  async fn t37_convert_different_formats_different_output() {
    ...
    let op = ConvertOp;
    let r_png = op.execute(serde_json::json!({"hash": hash, "format": "png"}), &ctx).await.unwrap();
    let r_jpeg = op.execute(serde_json::json!({"hash": hash, "format": "jpeg"}), &ctx).await.unwrap();
    let r_webp = op.execute(serde_json::json!({"hash": hash, "format": "webp"}), &ctx).await.unwrap();
    let d_png = if let MediaOpResult::Binary { data, .. } = r_png { data } else { panic!() };
    let d_jpeg = if let MediaOpResult::Binary { data, .. } = r_jpeg { data } else { panic!() };
    let d_webp = if let MediaOpResult::Binary { data, .. } = r_webp { data } else { panic!() };
  ...
  async fn t38_optimize_different_levels_different_sizes() {
    ...
    let op = crate::runtime::builtin::media::optimize::OptimizeOp;
    let r1 = op.execute(serde_json::json!({"hash": hash, "level": 1}), &ctx).await.unwrap();
    let r6 = op.execute(serde_json::json!({"hash": hash, "level": 6}), &ctx).await.unwrap();
    let s1 = if let MediaOpResult::Binary { data, .. } = r1 { data.len() } else { 0 };
    let s6 = if let MediaOpResult::Binary { data, .. } = r6 { data.len() } else { 0 };
  ...
  async fn t39_thumbnail_output_smaller_than_input() {
    ...
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 50}), &ctx).await.unwrap();
    let output_size = if let MediaOpResult::Binary { data, .. } = result { data.len() } else { 0 };
    assert!(
  ...
  async fn t40_optimize_output_never_larger() {
    ...
    let hash = store(&ctx, &original).await;
    let op = crate::runtime::builtin::media::optimize::OptimizeOp;
    let result = op.execute(serde_json::json!({"hash": hash, "level": 2}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { data, metadata, .. } = result {
      assert!(
  ...
  async fn t43_color_hex_always_lowercase_7chars() {
    ...
    let op = DominantColorOp;
    let result = op.execute(serde_json::json!({"hash": hash, "count": 5}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = result {
  ...
  async fn t45_color_count_matches_array_length() {
    ...
    let op = DominantColorOp;
    let result = op.execute(serde_json::json!({"hash": hash, "count": 8}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = result {
      let colors = v["colors"].as_array().unwrap();
      let count = v["count"].as_u64().unwrap();
      assert_eq!(count, colors.len() as u64, "count field should match array length");
    } else {
      panic!("expected Metadata");
    }
  }
  ...
  async fn t46_thumbhash_solid_vs_gradient_differ() {
    ...
    let op = ThumbhashOp;
    let r1 = op.execute(serde_json::json!({"hash": solid_hash}), &ctx).await.unwrap();
    let r2 = op.execute(serde_json::json!({"hash": grad_hash}), &ctx).await.unwrap();
    let h1 = if let MediaOpResult::Metadata(v) = r1 { v["thumbhash"].as_str().unwrap().to_string() } else { panic!() };
    let h2 = if let MediaOpResult::Metadata(v) = r2 { v["thumbhash"].as_str().unwrap().to_string() } else { panic!() };
    assert_ne!(h1, h2, "solid and gradient should have different thumbhashes");
  }
  ...
  async fn t48_thumbhash_of_thumbnail_deterministic() {
    ...
    let r = thumb_op.execute(serde_json::json!({"hash": hash, "width": 80}), &ctx).await.unwrap();
    let thumb_hash = if let MediaOpResult::Binary { data, .. } = &r { store(&ctx, data).await } else { panic!() };
    // Thumbhash twice
    let th_op = ThumbhashOp;
    let r1 = th_op.execute(serde_json::json!({"hash": thumb_hash}), &ctx).await.unwrap();
    let r2 = th_op.execute(serde_json::json!({"hash": thumb_hash}), &ctx).await.unwrap();
    let h1 = if let MediaOpResult::Metadata(v) = r1 { v["thumbhash"].as_str().unwrap().to_string() } else { panic!() };
    let h2 = if let MediaOpResult::Metadata(v) = r2 { v["thumbhash"].as_str().unwrap().to_string() } else { panic!() };
    assert_eq!(h1, h2);
  }
  ...
  async fn t49_color_quality_extremes_both_valid() {
    ...
    let op = DominantColorOp;
    for q in [1, 5, 10] {
      let result = op.execute(serde_json::json!({"hash": hash, "quality": q}), &ctx).await;
      assert!(result.is_ok(), "quality={q} should produce valid result");
      if let Ok(MediaOpResult::Metadata(v)) = result {
        assert!(!v["colors"].as_array().unwrap().is_empty(), "quality={q} should return colors");
      }
    }
  }
  ...
  fn t51_schema_dimensions_requires_hash() {
    ...
    assert!(required.iter().any(|v| v == "hash"), "dimensions schema must require 'hash'");
  }
  ...
  fn t52_schema_thumbnail_requires_hash_and_width() {
    ...
    assert!(req_strs.contains(&"width"), "thumbnail must require 'width'");
  }
  ...
  async fn t61_thumbnail_metadata_has_width_height() {
    ...
    if let MediaOpResult::Binary { metadata, .. } = result {
      assert!(metadata.get("width").is_some(), "metadata should have width");
      assert!(metadata.get("height").is_some(), "metadata should have height");
  ...
  async fn t62_convert_metadata_has_converted_to() {
    ...
    for fmt in ["png", "jpeg", "webp"] {
      let result = op.execute(serde_json::json!({"hash": hash, "format": fmt}), &ctx).await.unwrap();
      if let MediaOpResult::Binary { metadata, .. } = result {
        assert_eq!(metadata["converted_to"], fmt, "converted_to should be {fmt}");
      } else {
  ...
  async fn t64_optimize_metadata_all_fields() {
    ...
    let op = crate::runtime::builtin::media::optimize::OptimizeOp;
    let result = op.execute(serde_json::json!({"hash": hash, "level": 3}), &ctx).await.unwrap();
    if let MediaOpResult::Binary { metadata, .. } = result {
      ...
      assert!(savings >= 0.0 && savings <= 100.0, "savings_pct should be 0-100, got {savings}");
    } else {
      panic!("expected Binary");
    }
  }
  ...
  async fn t65_dimensions_metadata_has_orientation() {
    let (_dir, ctx) = setup().await;
    for (w, h, expected) in [(200, 100, "landscape"), (100, 200, "portrait"), (50, 50, "square")] {
      let hash = store(&ctx, &solid_rgba(w, h, 0, 0, 0, 255)).await;
      let op = DimensionsOp;
      let result = op.execute(serde_json::json!({"hash": hash}), &ctx).await.unwrap();
      if let MediaOpResult::Metadata(v) = result {
        assert_eq!(v["orientation"], expected, "{}x{} should be {expected}", w, h);
      } else {
        panic!("expected Metadata");
      }
    }
  ...
  async fn t67_thumbhash_response_fields() {
    ...
    if let MediaOpResult::Metadata(v) = result {
      ...
      assert!(v.get("size_bytes").is_some(), "should have size_bytes field");
      assert!(v["thumbhash"].is_string());
  ...
  async fn t68_color_response_structure() {
    ...
    let op = DominantColorOp;
    let result = op.execute(serde_json::json!({"hash": hash, "count": 4}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = result {
  ...
  async fn t69_media_op_result_variant_distinguishable() {
    ...
    let dim_result = DimensionsOp.execute(serde_json::json!({"hash": hash}), &ctx).await.unwrap();
    assert!(matches!(dim_result, MediaOpResult::Metadata(_)));
    // Thumbnail returns Binary
    let thumb_result = ThumbnailOp.execute(serde_json::json!({"hash": hash, "width": 5}), &ctx).await.unwrap();
  ...
  async fn t71_chain_thumb_strip_convert_dimensions() {
    ...
    let r1 = ThumbnailOp.execute(serde_json::json!({"hash": hash, "width": 60}), &ctx).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };

    // Step 2: Strip
    let r2 = StripOp.execute(serde_json::json!({"hash": h1}), &ctx).await.unwrap();
    let h2 = if let MediaOpResult::Binary { data, .. } = &r2 { store(&ctx, data).await } else { panic!() };

    // Step 3: Convert to JPEG
    let r3 = ConvertOp.execute(serde_json::json!({"hash": h2, "format": "jpeg"}), &ctx).await.unwrap();
    let h3 = if let MediaOpResult::Binary { data, .. } = &r3 { store(&ctx, data).await } else { panic!() };

    // Step 4: Dimensions
    let r4 = DimensionsOp.execute(serde_json::json!({"hash": h3}), &ctx).await.unwrap();
  ...
  async fn t72_chain_convert_jpeg_thumb_color() {
    ...
    // Convert to JPEG
    let r1 = ConvertOp.execute(serde_json::json!({"hash": hash, "format": "jpeg"}), &ctx).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };

    // Thumbnail
    let r2 = ThumbnailOp.execute(serde_json::json!({"hash": h1, "width": 20}), &ctx).await.unwrap();
    let h2 = if let MediaOpResult::Binary { data, .. } = &r2 { store(&ctx, data).await } else { panic!() };

    // Dominant color should still be green-ish
    let r3 = DominantColorOp.execute(serde_json::json!({"hash": h2}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = r3 {
      let first = &v["colors"].as_array().unwrap()[0];
      assert!(first["g"].as_u64().unwrap() > 150, "green should be dominant after chain");
    } else {
      panic!("expected Metadata");
    }
  }
  ...
  async fn t73_chain_thumbnail_thumbhash_valid() {
    ...
    let r1 = ThumbnailOp.execute(serde_json::json!({"hash": hash, "width": 50}), &ctx).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };

    let r2 = ThumbhashOp.execute(serde_json::json!({"hash": h1}), &ctx).await.unwrap();
  ...
  async fn t74_chain_optimize_thumbnail_dimensions() {
    ...
    // Optimize
    let r1 = crate::runtime::builtin::media::optimize::OptimizeOp.execute(
      serde_json::json!({"hash": hash}), &ctx
    ).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };

    // Thumbnail
    let r2 = ThumbnailOp.execute(serde_json::json!({"hash": h1, "width": 30}), &ctx).await.unwrap();
    let h2 = if let MediaOpResult::Binary { data, .. } = &r2 { store(&ctx, data).await } else { panic!() };

    // Dimensions
    let r3 = DimensionsOp.execute(serde_json::json!({"hash": h2}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = r3 {
  ...
  async fn t75_chain_double_strip_idempotent() {
    ...
    assert_eq!(d1, d2, "strip(strip(img)) should be identical to strip(img)");
  }
  ...
  async fn t76_chain_svg_render_thumb_color() {
    ...
    // Render
    let r1 = crate::runtime::builtin::media::svg::SvgRenderOp.execute(
      serde_json::json!({"hash": hash, "width": 80, "height": 80}), &ctx
    ).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };
    ...
    if let MediaOpResult::Metadata(v) = r3 {
      let first = &v["colors"].as_array().unwrap()[0];
      assert!(first["b"].as_u64().unwrap() > 150, "blue SVG -> blue dominant color");
    } else {
      panic!("expected Metadata");
    }
  }
  ...
  async fn t77_chain_thumbnail_webp_then_dimensions() {
    ...
    let r1 = ThumbnailOp.execute(
      serde_json::json!({"hash": hash, "width": 40, "format": "webp"}), &ctx
    ).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };

    let r2 = DimensionsOp.execute(serde_json::json!({"hash": h1}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = r2 {
  ...
  async fn t78_chain_jpeg_low_quality_to_optimized_png() {
    ...
    // Low quality JPEG
    let r1 = ConvertOp.execute(serde_json::json!({"hash": hash, "format": "jpeg", "quality": 5}), &ctx).await.unwrap();
    let h1 = if let MediaOpResult::Binary { data, .. } = &r1 { store(&ctx, data).await } else { panic!() };

    // JPEG -> PNG
    let r2 = ConvertOp.execute(serde_json::json!({"hash": h1, "format": "png"}), &ctx).await.unwrap();
    let h2 = if let MediaOpResult::Binary { data, .. } = &r2 { store(&ctx, data).await } else { panic!() };

    // Optimize PNG
    let r3 = crate::runtime::builtin::media::optimize::OptimizeOp.execute(
      serde_json::json!({"hash": h2}), &ctx
    ).await.unwrap();
    if let MediaOpResult::Binary { data, .. } = r3 {
      let img = image::load_from_memory(&data).unwrap();
      assert_eq!(img.width(), 60);
  ...
  async fn t79_chain_every_binary_tool_metadata_has_image_mime() {
    ...
    // Thumbnail
    let r = ThumbnailOp.execute(serde_json::json!({"hash": hash, "width": 10}), &ctx).await.unwrap();
    let h = if let MediaOpResult::Binary { data, .. } = &r { store(&ctx, data).await } else { panic!() };
    let m = meta_op.execute(serde_json::json!({"hash": h}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = m {
      assert!(v["mime_type"].as_str().unwrap().starts_with("image/"), "thumbnail output should be image/*");
    }

    // Convert
    let r = ConvertOp.execute(serde_json::json!({"hash": hash, "format": "jpeg"}), &ctx).await.unwrap();
    let h = if let MediaOpResult::Binary { data, .. } = &r { store(&ctx, data).await } else { panic!() };
    let m = meta_op.execute(serde_json::json!({"hash": h}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = m {
      assert!(v["mime_type"].as_str().unwrap().starts_with("image/"), "convert output should be image/*");
    }

    // Strip
    let r = StripOp.execute(serde_json::json!({"hash": hash}), &ctx).await.unwrap();
    let h = if let MediaOpResult::Binary { data, .. } = &r { store(&ctx, data).await } else { panic!() };
    let m = meta_op.execute(serde_json::json!({"hash": h}), &ctx).await.unwrap();
    if let MediaOpResult::Metadata(v) = m {
      assert!(v["mime_type"].as_str().unwrap().starts_with("image/"), "strip output should be image/*");
    }
  }
  ...
  async fn t80_all_tools_share_same_hash_without_interference() {
    ...
    let dim = DimensionsOp.execute(serde_json::json!({"hash": hash}), &ctx).await;
    let thumb = ThumbnailOp.execute(serde_json::json!({"hash": hash, "width": 30}), &ctx).await;
    let conv = ConvertOp.execute(serde_json::json!({"hash": hash, "format": "jpeg"}), &ctx).await;
    let strip = StripOp.execute(serde_json::json!({"hash": hash}), &ctx).await;
    let color = DominantColorOp.execute(serde_json::json!({"hash": hash}), &ctx).await;
    let thash = ThumbhashOp.execute(serde_json::json!({"hash": hash}), &ctx).await;
  ...
  async fn t81_concurrent_thumbnails_correct_dimensions() {
    ...
    let handles: Vec<_> = widths.iter().map(|&w| {
      let ctx = Arc::clone(&ctx);
      let hash = hash.clone();
      tokio::spawn(async move {
        let op = ThumbnailOp;
        let result = op.execute(serde_json::json!({"hash": hash, "width": w}), &ctx).await.unwrap();
        if let MediaOpResult::Binary { data, .. } = result {
          let img = image::load_from_memory(&data).unwrap();
    ...
    for r in results {
      ...
      assert_eq!(actual_w, requested_w, "width mismatch for requested {requested_w}");
      let expected_h = (requested_w as f64 * 100.0 / 200.0).round() as u32;
      assert_eq!(actual_h, expected_h, "height mismatch for width {requested_w}");
    }
  }
  ...
  async fn t82_concurrent_mixed_metadata_tools() {
    ...
    let (r1, r2, r3) = tokio::join!(
      tokio::spawn(async move { DimensionsOp.execute(serde_json::json!({"hash": h1}), &ctx1).await }),
      tokio::spawn(async move { DominantColorOp.execute(serde_json::json!({"hash": h2}), &ctx2).await }),
      tokio::spawn(async move { ThumbhashOp.execute(serde_json::json!({"hash": h3}), &ctx3).await }),
    );
  ...
  async fn t83_concurrent_converts_different_formats() {
    ...
    let formats = vec!["png", "jpeg", "webp"];
    let handles: Vec<_> = formats.iter().map(|&fmt| {
      let ctx = Arc::clone(&ctx);
      let hash = hash.clone();
  ...
  async fn t84_concurrent_stores_unique_hashes() {
    let (_dir, ctx) = setup().await;
    let images: Vec<Vec<u8>> = (0..10u8).map(|i| solid_rgba(10, 10, i * 25, 0, 0, 255)).collect();
    let handles: Vec<_> = images.iter().map(|img| {
      let ctx = Arc::clone(&ctx);
      let img = img.clone();
      tokio::spawn(async move { ctx.cas.store(&img).await.unwrap().hash })
    }).collect();
    let hashes: Vec<String> = futures::future::join_all(handles).await.into_iter().map(|r| r.unwrap()).collect();
    // All hashes should be unique
    let unique: std::collections::HashSet<&String> = hashes.iter().collect();
    assert_eq!(unique.len(), 10, "10 different images should have 10 unique hashes");
  }
  ...
  async fn t85_concurrent_thumbnail_and_strip_no_corruption() {
    ...
    let (r1, r2) = tokio::join!(
      tokio::spawn(async move {
        ThumbnailOp.execute(serde_json::json!({"hash": h1, "width": 30}), &ctx1).await
      }),
      tokio::spawn(async move {
        StripOp.execute(serde_json::json!({"hash": h2}), &ctx2).await
      }),
    );

    let thumb_data = if let MediaOpResult::Binary { data, .. } = r1.unwrap().unwrap() { data } else { panic!() };
    let strip_data = if let MediaOpResult::Binary { data, .. } = r2.unwrap().unwrap() { data } else { panic!() };
    ...
    assert_eq!(strip_img.width(), 100, "strip should preserve original 100px width");
  }
  ...
  async fn t86_concurrent_reads_identical() {
    ...
    let results: Vec<Vec<u8>> = futures::future::join_all(handles).await.into_iter().map(|r| r.unwrap()).collect();
    for r in &results {
  ...
  async fn t87_compute_pool_thread_name_verified() {
    let (_dir, ctx) = setup().await;
    let name = ctx.compute.compute(|| {
      std::thread::current().name().unwrap_or("unknown").to_string()
    }).await.unwrap();
    assert!(name.starts_with("nika-media"), "compute should run on nika-media-N thread, got: {name}");
  }
  ...
  async fn t91_thumbnail_width_at_max_10000_accepted() {
    ...
    let hash = store(&ctx, &solid_rgba(10, 10, 0, 0, 0, 255)).await;
    let op = ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": hash, "width": 10000}), &ctx).await;
  ...
  async fn t93_dimensions_webp_image() {
    ...
    let op = ConvertOp;
    let r = op.execute(serde_json::json!({"hash": hash, "format": "webp"}), &ctx).await.unwrap();
    let webp_hash = if let MediaOpResult::Binary { data, .. } = &r { store(&ctx, data).await } else { panic!() };
    let dim = DimensionsOp.execute(serde_json::json!({"hash": webp_hash}), &ctx).await.unwrap();
