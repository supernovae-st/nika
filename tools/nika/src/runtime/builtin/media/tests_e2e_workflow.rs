...
mod tests {
  ...
  async fn dispatch_dominant_color_through_router() {
    ...
    let first = &colors[0];
    assert!(first["r"].as_u64().unwrap() > 200, "red channel should be dominant");
  }
  ...
  async fn dispatch_unknown_tool_returns_error() {
    ...
    let result = router
      .dispatch("nika:nonexistent", "{}".to_string())
      .await;
  ...
  async fn adapter_valid_json_dispatches() {
    ...
    assert!(result.is_ok(), "valid JSON should succeed: {:?}", result.err());
    let parsed: serde_json::Value = serde_json::from_str(&result.unwrap()).unwrap();
  ...
  async fn adapter_binary_result_writes_to_cas() {
    ...
    assert!(cas_hash.starts_with("blake3:"), "result should contain CAS hash");
  ...
  async fn pipeline_thumbnail_then_optimize_still_valid_png() {
    ...
    assert_eq!(&data[..4], &[137, 80, 78, 71], "optimized output should be PNG");
  ...
  async fn pipeline_same_cas_hash_dedup_across_tools() {
    ...
    assert!(sr2.deduplicated, "second store should be flagged as deduplicated");
    ...
    let thumb_result = router
      ...
      .await;
    assert!(thumb_result.is_ok(), "thumbhash on deduped hash should work");
  ...
  async fn pipeline_concurrent_tools_on_same_hash() {
    ...
    let media_ctx = Arc::new(MediaToolContext::new(CasStore::new(dir.path())));
    let router = Arc::new(BuiltinToolRouter::with_all_tools(tool_ctx, Arc::clone(&media_ctx)));
    ...
    let handles: Vec<_> = (0..5)
      .map(|i| {
        ...
        tokio::spawn(async move {
          match i % 3 {
            0 => router
              .dispatch("nika:dimensions", serde_json::json!({"hash": hash}).to_string())
              .await,
            1 => router
              .dispatch("nika:thumbhash", serde_json::json!({"hash": hash}).to_string())
              .await,
            _ => router
              .dispatch("nika:dominant_color", serde_json::json!({"hash": hash}).to_string())
              .await,
          }
        })
      })
  ...
  async fn router_error_propagation_missing_cas_hash() {
    ...
    let tools = vec![
      ...
      ("nika:thumbnail", serde_json::json!({"hash": fake_hash, "width": 50})),
      ("nika:thumbhash", serde_json::json!({"hash": fake_hash})),
      ("nika:dominant_color", serde_json::json!({"hash": fake_hash})),
      ("nika:convert", serde_json::json!({"hash": fake_hash, "format": "jpeg"})),
  ...
  async fn pipeline_duplicate_thumbnail_is_deduplicated() {
    ...
    let result1 = router.dispatch("nika:thumbnail", args.clone()).await.unwrap();
