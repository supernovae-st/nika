...
mod tests {
  ...
  async fn pipeline_import_then_dimensions_png() {
    ...
    // Step 2: Dimensions
    let dims_result = DimensionsOp.execute(
  ...
  async fn pipeline_import_then_thumbhash() {
    ...
    let thumbhash_result = ThumbhashOp.execute(
      serde_json::json!({"hash": hash}),
      &ctx,
    ).await.unwrap();
    ...
    if let MediaOpResult::Metadata(v) = thumbhash_result {
      ...
      let decoded = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, th).unwrap();
      assert!(decoded.len() >= 3 && decoded.len() <= 28,
        "thumbhash decoded length should be 3-28, got {}", decoded.len());
    }
  }
  ...
  async fn pipeline_import_then_dominant_color() {
    ...
    let color_result = DominantColorOp.execute(
      serde_json::json!({"hash": hash}),
      &ctx,
    ).await.unwrap();
    ...
    if let MediaOpResult::Metadata(v) = color_result {
      ...
      assert!(!colors.is_empty(), "should have at least one dominant color");
    }
  }
  ...
  async fn pipeline_import_then_thumbnail() {
    ...
    let thumb_result = ThumbnailOp.execute(
      serde_json::json!({"hash": hash, "width": 100}),
      &ctx,
    ).await.unwrap();

    if let MediaOpResult::Binary { data, mime_type, metadata, .. } = thumb_result {
      assert_eq!(mime_type, "image/png");
      assert!(!data.is_empty());
      assert_eq!(metadata["width"], 100);
      // Thumbnail should be smaller than original
      assert!(data.len() < png.len(), "thumbnail should be smaller than original");
    } else {
  ...
  async fn pipeline_import_thumbnail_dimensions() {
    ...
    // Import
    let import_hash = extract_hash(ImportOp.execute(
      serde_json::json!({"path": tmp.path().to_string_lossy()}),
      &ctx,
    ).await.unwrap());
    ...
    // Thumbnail (width=200)
    let thumb_result = ThumbnailOp.execute(
      serde_json::json!({"hash": import_hash, "width": 200}),
      &ctx,
    ).await.unwrap();
    ...
    let dims = DimensionsOp.execute(
      serde_json::json!({"hash": import_hash}),
      &ctx,
    ).await.unwrap();
  ...
  async fn import_charges_budget() {
    ...
    assert_eq!(budget_after, png.len() as u64, "budget should be charged by file size");
  }
  ...
  async fn import_concurrent_same_file() {
    ...
    assert!(results.iter().all(|r| r.is_ok()), "all concurrent imports should succeed");

    // All should produce the same hash
    let hashes: Vec<String> = results.into_iter()
      .map(|r| extract_hash(r.unwrap()))
      .collect();
    assert!(hashes.windows(2).all(|w| w[0] == w[1]), "same file should produce same hash");
  }
  ...
  async fn pipeline_import_then_phash() {
    ...
    let hash = extract_hash(ImportOp.execute(
      serde_json::json!({"path": tmp.path().to_string_lossy()}),
      &ctx,
    ).await.unwrap());

    let phash_result = PhashOp.execute(
      serde_json::json!({"hash": hash}),
      &ctx,
    ).await.unwrap();
  ...
  async fn pipeline_import_two_images_then_compare() {
    ...
    let h2 = extract_hash(ImportOp.execute(
      serde_json::json!({"path": tmp2.path().to_string_lossy()}),
      &ctx,
    ).await.unwrap());

    let compare_result = CompareOp.execute(
      serde_json::json!({"hash_a": h1, "hash_b": h2}),
      &ctx,
    ).await.unwrap();
  ...
  async fn pipeline_import_then_optimize() {
    ...
    let opt_result = OptimizeOp.execute(
      serde_json::json!({"hash": hash}),
      &ctx,
    ).await.unwrap();

    if let MediaOpResult::Binary { data, mime_type, .. } = opt_result {
      assert_eq!(mime_type, "image/png");
      assert!(!data.is_empty());
      // Optimized solid-color PNG should be smaller or equal
      assert!(data.len() <= png.len() + 100,
  ...
  async fn pipeline_import_then_convert_png_to_jpeg() {
    ...
    let conv_result = ConvertOp.execute(
      serde_json::json!({"hash": hash, "format": "jpeg"}),
      &ctx,
    ).await.unwrap();

    if let MediaOpResult::Binary { data, mime_type, .. } = conv_result {
  ...
  async fn pipeline_import_then_metadata() {
    ...
    let meta_result = MetadataOp.execute(
      serde_json::json!({"hash": hash}),
      &ctx,
    ).await.unwrap();
