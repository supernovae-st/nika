...
mod tests {
  ...
  fn fixture_png_100x50() -> Vec<u8> {
    ...
    image::ImageEncoder::write_image(
      enc, img.as_raw(), 100, 50, image::ExtendedColorType::Rgba8,
    ).unwrap();
    buf
  }
  ...
  async fn integration_thumbnail_then_dimensions() {
    ...
    let result = thumb_op.execute(serde_json::json!({
    ...
    let dim_result = dim_op.execute(serde_json::json!({"hash": thumb_hash}), &ctx).await.unwrap();
  ...
  async fn integration_thumbnail_then_convert() {
    ...
    let result = convert_op.execute(serde_json::json!({
  ...
  async fn integration_strip_then_dimensions() {
    ...
    let result = strip_op.execute(serde_json::json!({"hash": sr.hash}), &ctx).await.unwrap();
    ...
    let dim_op = DimensionsOp;
    let result = dim_op.execute(serde_json::json!({"hash": stripped_hash}), &ctx).await.unwrap();
  ...
  async fn cas_concurrent_tool_writes() {
    ...
    let handles: Vec<_> = (0..10u8).map(|i| {
      let ctx = Arc::clone(&ctx);
  ...
  fn security_svg_safe_patterns() {
    ...
    for (i, svg) in safe_svgs.iter().enumerate() {
      ...
      assert!(result.is_ok(), "safe SVG {i} should pass: {:?}", result.err());
    }
  }
  ...
  async fn feature_all_tools_registered() {
    ...
    assert!(names.contains(&"dimensions"), "dimensions should always be registered");
    assert!(names.contains(&"thumbhash"), "thumbhash should always be registered");
    assert!(names.contains(&"dominant_color"), "dominant_color should always be registered");

    // Feature-gated tools
    #[cfg(feature = "media-thumbnail")]
    {
      assert!(names.contains(&"thumbnail"), "thumbnail should be registered with media-thumbnail");
      assert!(names.contains(&"convert"), "convert should be registered with media-thumbnail");
      assert!(names.contains(&"strip"), "strip should be registered with media-thumbnail");
    }
    #[cfg(feature = "media-metadata")]
    assert!(names.contains(&"metadata"), "metadata should be registered with media-metadata");
    #[cfg(feature = "media-optimize")]
    assert!(names.contains(&"optimize"), "optimize should be registered with media-optimize");
    #[cfg(feature = "media-svg")]
    assert!(names.contains(&"svg_render"), "svg_render should be registered with media-svg");
  }
  ...
  async fn router_dispatches_media_tools() {
    ...
    use crate::tools::{ToolContext, PermissionMode};
    use std::path::PathBuf;

    let dir = tempfile::tempdir().unwrap();
    let tool_ctx = Arc::new(ToolContext::new(dir.path().to_path_buf(), PermissionMode::YoloMode));
    ...
    assert!(router.has_tool("dominant_color"), "dominant_color not in router");

    // nika: prefix dispatch works
    assert!(crate::runtime::builtin::BuiltinToolRouter::is_builtin("nika:dimensions"));
  }
  ...
  async fn router_has_all_expected_tools() {
    ...
    assert!(names.len() >= 15, "expected >= 15 tools, got {}: {:?}", names.len(), names);
  }
}
