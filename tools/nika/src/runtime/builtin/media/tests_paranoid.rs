...
mod tests {
  ...
  use crate::runtime::builtin::media::{create_media_tool_adapters, MediaOp, MediaOpResult};
  ...
  async fn params_dimensions_extra_unknown_fields() {
    let (_dir, ctx) = setup().await;
    let data = vec![137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 0, 5, 0, 0, 0, 5, 8, 2, 0, 0, 0];
  ...
  async fn params_color_quality_boundary_values() {
    ...
    let sr = ctx.cas.store(data).await.unwrap();
    let result = op.execute(serde_json::json!({"hash": sr.hash, "quality": 0}), &ctx).await;
    ...
    let result = op.execute(serde_json::json!({"hash": sr.hash, "quality": 100}), &ctx).await;
  ...
  async fn params_thumbnail_width_as_float() {
    ...
    let result = op.execute(serde_json::json!({"hash": "blake3:abc", "width": 50.5}), &ctx).await;
  ...
  async fn params_thumbnail_width_as_negative() {
    let (_dir, ctx) = setup().await;
    let op = crate::runtime::builtin::media::thumbnail::ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": "blake3:abc", "width": -50}), &ctx).await;
  ...
  async fn params_thumbnail_width_as_string() {
    let (_dir, ctx) = setup().await;
    let op = crate::runtime::builtin::media::thumbnail::ThumbnailOp;
    let result = op.execute(serde_json::json!({"hash": "blake3:abc", "width": "fifty"}), &ctx).await;
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("NIKA-294"));
  }
...
<svg xmlns="http://www.w3.org/2000/svg"><text>&lol2;</text></svg>"#;
  ...
  async fn stress_100_concurrent_dimensions() {
    ...
    let data = {
      ...
      image::ImageEncoder::write_image(enc, img.as_raw(), 10, 10, image::ExtendedColorType::Rgb8).unwrap();
      buf
    };
    #[cfg(not(feature = "media-thumbnail"))]
    let data = vec![
      137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82,
      0, 0, 0, 10, 0, 0, 0, 10, 8, 2, 0, 0, 0,
    ];
    let sr = ctx.cas.store(&data).await.unwrap();
    ...
    assert!(ok_count >= 95, "expected >= 95 successes from 100 concurrent ops, got {ok_count}");
  }
  ...
  async fn stress_budget_exhaustion_and_recovery() {
    ...
    let ctx = Arc::new(MediaToolContext {
      ...
      working_memory: Arc::new(crate::runtime::builtin::media::context::WorkingMemoryBudget::new()),
    ...
    assert!(err_msg.contains("NIKA-259"), "expected budget error, got: {err_msg}");
  ...
  async fn adapter_all_tools_have_name() {
    ...
    for tool in &tools {
      ...
      assert!(!name.contains(':'), "tool name must not contain colon: {name}");
    }
  }
  ...
  async fn adapter_all_tools_have_description() {
    ...
    for tool in &tools {
      ...
      assert!(!desc.is_empty(), "tool {} must have description", tool.name());
    }
  }
  ...
  async fn adapter_all_tools_have_schema() {
    ...
    for tool in &tools {
      ...
      assert!(schema.is_object(), "tool {} schema must be object", tool.name());
      let obj = schema.as_object().unwrap();
      assert!(obj.contains_key("properties"), "tool {} must have properties", tool.name());
      assert!(obj.contains_key("required"), "tool {} must have required", tool.name());
    }
  }
  ...
  async fn adapter_all_tools_reject_empty_json() {
    ...
    for tool in &tools {
      ...
      assert!(result.is_err(), "tool {} should reject empty params", tool.name());
    }
  }
  ...
  async fn adapter_all_tools_reject_invalid_json() {
    ...
    for tool in &tools {
      ...
      assert!(result.is_err(), "tool {} should reject invalid JSON", tool.name());
      let err = result.unwrap_err().to_string();
      assert!(err.contains("NIKA-294"), "tool {} should return NIKA-294, got: {err}", tool.name());
    }
  }
  ...
  async fn adapter_cancelled_context_rejects_all_tools() {
    ...
    for tool in &tools {
      ...
      assert!(result.is_err(), "tool {} should fail on cancelled context", tool.name());
    }
  }
  ...
  async fn enrichment_different_sizes_different_hashes() {
    ...
    let img2 = {
      ...
      let mut buf = Vec::new();
      let enc = image::codecs::png::PngEncoder::new(&mut buf);
      image::ImageEncoder::write_image(enc, img.as_raw(), 20, 20, image::ExtendedColorType::Rgb8).unwrap();
      base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &buf)
    };
      ...
      for i in 1..20u8 {
        ...
        if let Ok(sr) = ctx.cas.store(&data).await {
          let result = tool.call(
            serde_json::json!({"hash": sr.hash, "width": 50, "format": "png"}).to_string()
          ).await;
          if let Err(e) = &result {
            assert!(
