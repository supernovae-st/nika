...
mod tests {
  ...
  fn bomb_png_65535x65535_ihdr_rejected_by_limits() {
    ...
    let fake_idat = vec![0x78, 0x01, 0x01, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x01];
  ...
  fn bomb_png_valid_header_massive_idat_rejected() {
    ...
    let result = decode_image_safe(&png);
    assert!(result.is_err(), "10000x10000 RGBA PNG (400MB decoded) must be rejected");
    let err = result.unwrap_err();
    assert!(
      err.to_string().contains("NIKA-290"),
  ...
  fn bomb_truncated_png_signature_only() {
    ...
    assert!(result.is_err(), "truncated PNG (signature only) must be rejected");
  }
  ...
  fn bomb_png_at_dimension_limit_10000x1_accepted() {
    ...
    let img = image::ImageBuffer::from_pixel(
      10_000, 1, image::Rgba([255u8, 0, 0, 255])
    );
    let mut buf = Vec::new();
    let enc = image::codecs::png::PngEncoder::new(&mut buf);
    image::ImageEncoder::write_image(
      enc, img.as_raw(), 10_000, 1, image::ExtendedColorType::Rgba8,
    ).unwrap();
...
<svg xmlns="http://www.w3.org/2000/svg">
  ...
  fn svg_attack_ssrf_xlink_cloud_metadata() {
    ...
    let result = sanitize_svg(svg);
    assert!(result.is_err(), "xlink:href to cloud metadata must be blocked");
    assert!(result.unwrap_err().to_string().contains("NIKA-297"));
  }
  ...
  fn svg_attack_nested_svg_recursion() {
    ...
    assert!(result.is_ok(), "deeply nested SVG has no forbidden elements");
  }
  ...
  fn svg_attack_mixed_safe_and_dangerous() {
    ...
    let result = sanitize_svg(svg);
    assert!(result.is_err(), "single dangerous element in otherwise safe SVG must be rejected");
    assert!(result.unwrap_err().to_string().contains("NIKA-297"));
  }
  ...
  fn svg_attack_event_handler_whitespace_obfuscation() {
    ...
    let result = sanitize_svg(svg);
    assert!(result.is_err(), "event handler with extra whitespace must be caught");
    assert!(result.unwrap_err().to_string().contains("NIKA-297"));
  }
  ...
  fn svg_attack_event_handler_tab_separated() {
    ...
    let result = sanitize_svg(svg);
    assert!(result.is_err(), "event handler with tab separator must be caught");
    assert!(result.unwrap_err().to_string().contains("NIKA-297"));
  }
  ...
  fn svg_attack_javascript_mixed_case() {
    ...
    let result = sanitize_svg(svg);
    assert!(result.is_err(), "javascript: with mixed case must be caught");
    assert!(result.unwrap_err().to_string().contains("NIKA-297"));
  }
  ...
  fn svg_attack_foreign_object_mixed_case() {
    ...
    let result = sanitize_svg(svg);
    assert!(result.is_err(), "FOREIGNOBJECT must be caught case-insensitively");
    assert!(result.unwrap_err().to_string().contains("NIKA-297"));
  }
  ...
  fn svg_attack_multiple_forbidden_patterns() {
    ...
    let result = sanitize_svg(svg);
    assert!(result.is_err(), "SVG with multiple attacks must be rejected");
    assert!(result.unwrap_err().to_string().contains("NIKA-297"));
  }
  ...
  async fn path_traversal_dotdot_in_hash() {
    ...
    for hash in traversal_hashes {
      ...
      assert!(result.is_err(), "path traversal hash '{}' must be rejected", hash);
      let err_msg = result.unwrap_err().to_string();
      assert!(
        err_msg.contains("NIKA-253"),
        "expected NIKA-253 for path traversal '{}', got: {err_msg}", hash
      );
    }
  }
  ...
  async fn path_traversal_url_encoded() {
    ...
    for hash in hashes {
      let result = store.read(hash).await;
      assert!(result.is_err(), "URL-encoded traversal '{}' must be rejected", hash);
  ...
  async fn path_traversal_unicode_normalization() {
    ...
    for hash in hashes {
      let result = store.read(hash).await;
      assert!(result.is_err(), "Unicode normalization attack '{}' must be rejected", hash);
      assert!(result.unwrap_err().to_string().contains("NIKA-253"));
    }
  }
  ...
  async fn exhaustion_100_concurrent_operations() {
    ...
    let enc = image::codecs::png::PngEncoder::new(&mut buf);
    image::ImageEncoder::write_image(
      enc, img.as_raw(), 10, 10, image::ExtendedColorType::Rgba8,
    ).unwrap();
    ...
    let success_count = results.iter()
      ...
      .count();
    assert_eq!(success_count, 100, "all 100 concurrent dimension reads should succeed");
  }
  ...
  async fn exhaustion_budget_concurrent_race_condition() {
    ...
    let success_count = results.iter()
      .filter(|r| r.as_ref().unwrap().is_ok())
      .count();
    let fail_count = results.iter()
      .filter(|r| r.as_ref().unwrap().is_err())
      .count();

    assert_eq!(success_count, 10, "exactly 10 should fit in 1000-byte budget");
    assert_eq!(fail_count, 10, "exactly 10 should be rejected");
    assert_eq!(ctx.budget.current_bytes(), 1000, "budget should be exactly at max");
  }
  ...
  fn exhaustion_working_memory_concurrent_acquires() {
    ...
    assert_eq!(guards.len(), 10, "should fit 10 x 100 = 1000 in 1024 budget");
  ...
  fn exhaustion_working_memory_exact_then_one_more() {
    ...
    assert!(result.is_err(), "1 byte over working memory limit must fail");
    assert!(result.unwrap_err().to_string().contains("working memory exhausted"));
  }
  ...
  fn exhaustion_working_memory_zero_acquire() {
    ...
    assert!(result.is_ok(), "zero-size acquire should succeed even with zero budget");
  }
  ...
  async fn inject_dimensions_very_long_hash() {
    ...
    let result = op.execute(serde_json::json!({"hash": long_hash}), &ctx).await;
    assert!(result.is_err(), "1MB hash must be rejected (file not found)");
  ...
  async fn inject_convert_format_null_byte() {
    ...
    assert!(result.is_err(), "format with null byte must not match 'png'");
    assert!(
      result.unwrap_err().to_string().contains("NIKA-291"),
  ...
  async fn inject_all_tools_missing_hash_param() {
    ...
    let result = crate::runtime::builtin::media::color::DominantColorOp
      .execute(serde_json::json!({}), &ctx).await;
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("NIKA-294"));
  }
  ...
  async fn inject_array_hash_value() {
    let (_dir, ctx) = setup().await;
    let op = DimensionsOp;
    let result = op.execute(serde_json::json!({"hash": [1, 2, 3]}), &ctx).await;
  ...
  async fn inject_adapter_malformed_json() {
    let dir = tempfile::tempdir().unwrap();
    let ctx = Arc::new(MediaToolContext::new(CasStore::new(dir.path())));
    let adapter = MediaToolAdapter::new(
      Arc::new(DimensionsOp),
      ctx,
    );
    ...
    for input in malformed_inputs {
      ...
      assert!(result.is_err(), "malformed JSON '{}' must be rejected", input);
      let err = result.unwrap_err();
      assert!(
        err.to_string().contains("NIKA-294"),
        "expected NIKA-294 for malformed JSON '{}', got: {err}", input
      );
    }
  }
  ...
  async fn inject_optimize_negative_level() {
    ...
    assert!(result.is_ok(), "negative level should fall back to default, not crash");
  }
  ...
  async fn inject_svg_render_negative_dimensions() {
    ...
    assert!(result.is_ok(), "negative dimensions should be ignored (as_u64 returns None)");
  }
  ...
  async fn inject_convert_extreme_quality_values() {
    ...
    assert!(result.is_ok(), "quality=0 should be clamped to 1, not error");

    // Quality = 999 should be clamped to 100
    let result = op.execute(serde_json::json!({
      "hash": sr.hash, "format": "jpeg", "quality": 999
    }), &ctx).await;
    assert!(result.is_ok(), "quality=999 should be clamped to 100, not error");
  }
  ...
  async fn inject_dominant_color_extreme_count() {
    ...
    assert!(result.is_ok(), "count=1000 should be clamped to 20, not panic");
  }
