...
impl TaskExecutor {
    ...
    async fn run_infer(
        ...
        self.policy_enforcer.write().record_token_spend(actual_tokens);
    ...
    async fn run_agent(
        ...
        let estimated_tokens: u64 = resolved_agent.token_budget.map(u64::from).unwrap_or_else(|| {
        ...
        // Record actual token spend (v0.13.1)
        self.policy_enforcer.write().record_token_spend(result.total_tokens as u64);
...
mod tests {
    ...
    async fn test_execute_exec_blocked_by_policy() {
        ...
        match result.unwrap_err() {
            NikaError::PolicyViolation { reason } => {
                assert!(reason.contains("sudo"), "Reason should mention blocked pattern");
            }
    ...
    async fn test_execute_exec_disabled_by_policy() {
        ...
        match result.unwrap_err() {
            NikaError::PolicyViolation { reason } => {
                assert!(reason.contains("disabled"), "Reason should mention disabled");
            }
            err => panic!("Expected PolicyViolation, got: {err:?}"),
        }
    }
    ...
    async fn test_execute_fetch_disabled_by_policy() {
        ...
        assert!(result.is_err(), "Should be blocked when network is disabled");
        match result.unwrap_err() {
            NikaError::PolicyViolation { reason } => {
                assert!(reason.contains("disabled"), "Reason should mention disabled");
            }
