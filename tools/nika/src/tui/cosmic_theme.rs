...
impl CosmicTheme {
    ...
    /// # Performance
    ///
    /// This method creates a new Theme struct each call. For hot paths,
    /// cache the result or migrate to use `resolver()` directly.
...
mod tests {
    ...
    fn test_cosmic_theme_label() {
        let cosmic = CosmicTheme::new(CosmicVariant::CosmicDark);
        assert_eq!(cosmic.label(), "Cosmic Dark");

        let cosmic = CosmicTheme::new(CosmicVariant::CosmicLight);
        assert_eq!(cosmic.label(), "Cosmic Light");

        let cosmic = CosmicTheme::new(CosmicVariant::CosmicViolet);
        assert_eq!(cosmic.label(), "Cosmic Violet");
    }
    ...
    fn test_cosmic_variant_to_theme_mode() {
        assert_eq!(
            ThemeMode::from(CosmicVariant::CosmicDark),
            ThemeMode::Dark
        );
        assert_eq!(
