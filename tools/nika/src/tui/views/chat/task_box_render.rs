//! TaskBox variant rendering for Chat View
//!
//! Extracted from messages.rs — renders all 5 TaskBox variants (Invoke, Infer, Exec, Fetch, Agent)
...
/// Shared color configuration for task box rendering
...
impl ChatView {
    /// Render a TaskBox into the items list, dispatching to the appropriate variant renderer
    pub(super) fn render_task_box_items<'a>(
        &self,
        ...
        match task_box {
            ...
            TaskBox::Exec(exec) => {
                self.render_exec_box(items, exec, task_box, border_color, state_icon, state_suffix, colors);
            }
            TaskBox::Fetch(fetch) => {
                self.render_fetch_box(items, fetch, task_box, border_color, state_icon, state_suffix, colors);
            }
            TaskBox::Agent(agent) => {
                self.render_agent_box(items, agent, task_box, border_color, state_icon, state_suffix, colors);
            }
        }
    }
